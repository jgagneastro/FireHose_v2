...
function co_refract_list, a, altitude=altitude, pressure=pressure,  $
...
; expand those to be arrays of the same size.
	P = pressure + a*0. & T = temperature + a*0.
        ...
        cur = a + dr ; guess of observed location

        repeat begin
          last = cur
          dr = co_refract_forward(cur,P=pressure,T=temperature)
          cur= a + dr
        endrep until max(abs(last-cur)*3600.) LT epsilon
        aout = cur
endelse
