...
;
; CALLING SEQUENCE:
;   batchpbs_queue, [ platenums, topdir=, run2d=, run1d=, platestart=, plateend=, $
...
;   pbs_alloc  - If set, use #PBS -A pbs_alloc, otherwise
;                default to none
...
;                pbs_alloc = 'bolton-em' (Bolton's account, limited to 12 nodes: ember253-260,377-380)
;                pbs_walltime='336:00:00'
;   kingspeak  - If set, then setup the defaults for the kingspeak cluster at the University of Utah:
;                pbs_nodes = 16 (for 12 nodes, without node sharing) 
;                pbs_ppn = 16 (16 processors per node)
;                ;pbs_alloc = 'sdss-kp' (SDSS account, limited to 30 nodes: kp01-kp30)
;                pbs_walltime='336:00:00'
...
;   20-Sep-2013  Added pbs_queue support, Joel Brownstein Utah
;-
;------------------------------------------------------------------------------
...
; Do not use spPlancomb files that only have a subset of the MJDs !!!???
   ...
   scriptfile = fullscriptfile[ibatch]
   ...
   if keyword_set(pbs_batch) then begin
      ...
      for i=0L, nbatch-1L do begin
         thisfile = fileandpath(scriptfile[i], path=thispath)
