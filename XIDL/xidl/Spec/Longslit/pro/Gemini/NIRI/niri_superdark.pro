...
;-
;------------------------------------------------------------------------------
FUNCTION niri_superdark, filenames, sigrej = sigrej, maxiter = maxiter $
                         , verbose = verbose, NIRSPEC = NIRSPEC $
                         , TSPEC = TSPEC, ISAAC = ISAAC
...
;     Read this file
    IF KEYWORD_SET(NIRSPEC) THEN $
       nirspec_proc, filenames[ifile], image1, hdr = hdr1 $
                     , verbose = verbose $
    ELSE IF KEYWORD_SET(ISAAC) THEN $
       isaac_proc, filenames[ifile], image1, hdr = hdr1 $
                     , verbose = verbose $
    ELSE IF KEYWORD_SET(TSPEC) THEN $
       tspec_proc, filenames[ifile], image1, hdr = hdr1 $
                   , verbose = verbose $
    ELSE  niri_proc, filenames[ifile], image1, hdr = hdr1 $
                     , verbose = verbose 
