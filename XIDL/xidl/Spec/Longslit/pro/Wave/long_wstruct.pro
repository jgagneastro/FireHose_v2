...
ENDIF ELSE IF strmatch(instrument,'FORS2*') THEN BEGIN
   ...
   CASE grism OF
      ...
      'GRIS_1200B': BEGIN        
         ...
         wstruct.AUTOID      = 0
         wstruct.REID        = 1
         wstruct.REID_FILE   = calib_path + '/fors_1200B+97_mswave.sav'
         ;wstruct.REID_FILE   = calib_path + '/fors_300V+10_mswave.sav'
     END
     'GRIS_600V': BEGIN
         wstruct.pkwdth       = 6.0/(binning[1]/2.0d)
         ...
         wstruct.REID        = 1
         ;wstruct.REID_FILE   = calib_path + '/fors_600B+22_mswave.sav'
         wstruct.REID_FILE   = '/export/qso3/database/redux/FORS2/Rubin/P90-GPG/fors_600V+94_mswave.sav'
      END
     'GRIS_600RI': BEGIN
         wstruct.pkwdth       = 6.0/(binning[1]/2.0d)
         wstruct.TOLER        = 2.0/(binning[1]/2.0d)
...
ENDIF ELSE IF strmatch(telescope, '*SOAR*') THEN BEGIN
    ...
    CASE grating OF
       ...
       ELSE: message, 'ERROR: Unknown grating'
    ENDCASE
 ENDIF ELSE IF strmatch(telescope, '*ESO-NTT*') THEN BEGIN
