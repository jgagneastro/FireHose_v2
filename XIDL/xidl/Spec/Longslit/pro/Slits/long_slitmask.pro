...
pro long_slitmask, filename, outfile, biasfile = biasfile $
                   ...
                   , YRMASK = yrmask $
                   , HEFIXES = hefixes, slitmask = slitmask $
                   , SLIT_EDG_TWK = SLIT_EDG_TWK


;; JFH 09.09.2013, am going to go ahead and make the HEFIXES the
;; default behavior
HEFIXES = 1L
...
IF strmatch(telescope, '*DuPont*') THEN BEGIN
   peakthresh = 0.1
   minslit = 300L
endif

;;MF adding this quick fix for Palomar
IF strmatch(telid, '200') THEN BEGIN
   peakthresh = 0.1
   nfind1 = 1
endif
...
;;MF commenting this off for now 
;ELSE IF strmatch(telid, '200') THEN $
;   tset_slits = palomar_dbsp_slitset(nx, ny, fpa) $
ELSE IF strmatch(instrument, '*TWIN*') THEN $
   tset_slits = caha_slitset(nx, ny, hdr) $
ELSE IF strmatch(instrument, 'CAFOS2.2') THEN $
   tset_slits = caha_cafos_slitset(nx, ny) $
ELSE IF strmatch(instrument, 'bcspeclamps*',/fold) THEN $ ; jm11jun08ucsd
   tset_slits = bok_slitset(nx, ny, xtrim=xtrim) $
ELSE IF strmatch(telescope, '*Gemini-North*') AND KEYWORD_SET(GMOSLONG) $
THEN tset_slits = gmos_slitset(nx, ny) $
;ELSE IF strmatch(telescope, '*LBT-SX*') AND strmatch(obsmask, '*LS*') $
;THEN tset_slits = mods_slitset(nx, ny, CHANNEL = instrument) $
...
;   strmatch(fpa, 'DBSP_RED') THEN $
   ...
   if (NOT keyword_set(peakthresh)) then begin
      if not keyword_set(HEFIXES) then peakthresh = 0.02 else peakthresh = 0.03 ;; HE
; HE: Reduces the risk of dips in brightness caused by
; imperfectly milled slits being interpreted as gaps between slits,
; thereby causing slits to be broken up:
   endif
   ...
   ;; If ISAAC then zero out the amplifier difference that leads to spurious trace
   IF strmatch(instrument, '*ISAAC*') THEN cimg[490:530, *] = 0.0d
...
;       y2 = max(lightinds) <  (ny-1L)
   ...
   ENDIF ELSE BEGIN
      ...
      ;; HE fix to find spread out slits
      if keyword_set(HEFIXES) then begin
         fsum = fsum*0d0
         for i=0,2 do begin
;         create profile along x axis at four evenly spaced y positions:
            y_tmp = (i+1)*ny/4.
            y1 = y_tmp-0.05*ny > 0L
            y2 = y_tmp+0.05*ny < (ny-1L)
            ;; Smash center of the sharpness-filtered image to a 1-dimensional vector
            ymid = 0.5 * (y1 + y2)
            ftmp = total(cimg[*, y1:y2], 2)
;         compute SUM of all profiles in order to catch slits at far edges
;         of the mask that are not illuminated at all y positions:
            fsum = fsum+ftmp
         endfor
         ;; The above change in the definition of fsum can cause multiple
         ;; starting and ending positions for a given slit - fortunately, these
         ;; duplicate detections are caught and removed later!
      endif

;
      ;; JXP -- Original appears below.  Replaced by HE modification
      ;;        which looks good.  19 November 2012
     ...
     ; peakval = 1
      ...
      xmax = long_find_nminima(-fsum, nfind = 1, minsep = minsep, width = 3)
      peakcut = peakthresh*interpolate(-fsum, xmax)
      inoise = where(abs(fsum) lt abs(peakcut))  ;HE      
      ftmp = fsum & ftmp[inoise] = 0.
      xstart = long_find_nminima(-ftmp < peakcut, nfind = nfind $
                                 , minsep = minsep $
                                 , width = 3)
      ;; Find candidate ending positions
      xmax = long_find_nminima(fsum, nfind = 1, minsep = minsep, width = 3)
      ...
      inoise = where(abs(fsum) lt abs(peakcut))  ;HE      
      ftmp = fsum & ftmp[inoise] = 0.
      xend = long_find_nminima(ftmp < peakcut, nfind = nfind, minsep = minsep $
                               , width = 3)
      ...
      if n_elements(xstart) GT 1 then begin
         iclose = where(abs(round(xstart-shift(xstart,1))) lt minsep,nclose)
         if nclose gt 0 then begin
            xstart[iclose-1] = (xstart[iclose-1]+xstart[iclose])/2.
            remove,iclose,xstart
         endif
      endif
      if n_elements(xend) GT 1 then begin
         iclose = where(abs(round(xend-shift(xend,1))) lt minsep,nclose)
         if nclose gt 0 and n_elements(xend) GT 1 then begin
            xend[iclose-1] = (xend[iclose-1]+xend[iclose])/2.
            remove,iclose,xend
         endif
      endif
      nstart = n_elements(xstart)
...
;stop
  ...
  ;; Tweak slit boundaries. This step is recommended if you have very 
  ;; narrow slits
  IF KEYWORD_SET(SLIT_EDG_TWK) THEN BEGIN
     ;;chk = 1
     tset_slits0 = tset_slits
     FOR iter = 1L, 2L DO BEGIN
        splog, 'Tweaking slit boundaries iteration #', iter, ' of 2'
        tset_slits = long_slittweak(image, tset_slits0, chk = chk)
        tset_slits0 = tset_slits
     ENDFOR
  ENDIF
