...
IF KEYWORD_SET(IR) THEN yrange = [(0.5*djs_median(arc1d)) > 1.0 $
                                  , 1.3*max(djs_median(arc1d, width = 5))] $
ELSE yrange = [(0.05*djs_median(arc1d)) > 1.0 $
...
djs_plot, wvfit, abs(arc1d), xrange = wrange $
          ...
          , charsize = charsize, ylog = (KEYWORD_SET(IR) EQ 0)
FOR k = 0L, nlines-1L DO BEGIN
    tind = WHERE(k EQ rejpt, nt)
    IF nt EQ 0 THEN this_color = 'blue' $
    ELSE this_color = 'red'
    IF KEYWORD_SET(IR) THEN BEGIN
       w_string = strcompress(string(wlines[k].WAVE, FORMAT = '(F7.5)') $
                              , /rem) 
       xy = 0.95
       xy2 = 0.75
       xy3 = 0.7
    ENDIF ELSE BEGIN
       w_string = strcompress(string(wlines[k].WAVE, FORMAT = '(F8.2)'), /rem)
       xy = 0.5d
       xy2 = 0.1
       xy3 = 0.03
    ENDELSE
