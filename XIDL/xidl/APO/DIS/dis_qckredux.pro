...
pro dis_qckredux, img, spec, sig, FLAT=flat, CLINE=cline, APER=aper, $
                  ...
                  FULL=full, SAPER=saper, NOFLAT=noflat, BLUE=blue


;
  if  N_params() LT 2  then begin 
    print,'Syntax - ' + $
      'dis_qckredux, img, spec, [sig], FLAT=flat, CLINE=, APER=, ' + $
      'SKYFUNC=, SKYNORD=, '
    print, '        SKYREG=, /DISPLAY, /NOREJ, TRACE=, /NOSKY, STRCT=,'
    print, '        VAR=, /crude, /tinter, /SILENT, /CHKTRC ) [V1.1]'
    return
  endif 

;  Optional Keywords

  if not keyword_set(SAPER) then saper = 7.
  if not keyword_set(GRISM) then grism = '400'
  if not keyword_set(RN) then begin
      if keyword_set(BLUE) then rn = 4.61 else rn = 4.12
  endif
  if not keyword_set(NSKY) then nsky = [40., 40.]
  if not keyword_set(FULL) then icut = 200L ELSE icut = 275L

; Process
  print, 'dis_qckredux: Processing the image'
  if not keyword_set(FLAT) then NOFLAT=1
  dis_proc, img, finimg, flat, BLUE=blue, NOFLAT=noflat
  finimg = transpose(finimg)
  sz = size(finimg, /dimensions)

; OBJLIN
  if not keyword_set(objlin) then objlin = 383. 
  ...
  if keyword_set( strct ) then begin
      ...
      ; CLINE
      if not keyword_set( CLINE ) then  cline = 1370L
  endelse
