...
;               define bounds)
;  ncsiglim= -- number of flux error above continuum to includes
;               (default: 1)
...
function sdss_ewciv, wave, flux, sig, conti, dblt_name, zabs, istrct=istrct, $
  ...
  if keyword_set(contilim) then $
     ...
     if not keyword_set(ncsiglim) then ncsiglim = 1.
     flux =  flux0 < (conti + ncsiglim*abs(sig)) ; default 
  endelse 
;  flux = flux > (-1.*abs(sig))       ;  duh!
