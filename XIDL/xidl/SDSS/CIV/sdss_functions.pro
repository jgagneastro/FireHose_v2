...
function sdss_histogram, data, n_per_bin, min=min, max=max, scale=scale, $
  ...
  if keyword_set(scale) then $
     stop,'sdss_histogram() stop: scale= option does not work currently.'
  ...
  if not keyword_set(force) then begin
     ...
     histogram = sdss_histogram(srtdata,round(nmean),min=min,max=max,nbin=nbin,$
                                location=location,szloc=szloc,scale=scale,$
                                debug=debug,/force)
     if keyword_set(debug) then $
     ...
     if keyword_set(debug) then $
     print,'sdsss_histogram() debug: forcing bins as is.'
  endelse
  ...
  for tt=0L,nnewtags-1 do begin
     ...
     if mtch[0] eq -1 then begin
        ...
        if test[0] eq -1 then begin
           ...
           if ndim_new eq ndim_old then $
              newstrct.(tt) = oldstrct.(mtch[0]) $ ; actual copy
           else begin 
              if ndim_new gt ndim_old then begin
                 ...
                 if ndim2_old eq 1 then $
                    for ee=0,ndim_old-1 do $
                       newstrct.(tt)[ee] = oldstrct.(mtch[0])[ee] $ ; element-by-element copy
                 else begin
                    for ff=0,ndim2_old-1 do $ ; faster to have less looping outside
                       for ee=0,ndim_old-1 do $
                          newstrct.(tt)[ee,ff] = oldstrct.(mtch[0])[ee,ff]  ; element-by-element copy
                 endelse 
              endif else begin
...
function sdss_calcsigpoiss, number, sigma=sigma, cl=cl, verbose=verbose
  ...
  if not keyword_set(cl) then begin
     cl = gauss_pdf(1.)         ; = 0.841345
;     cl = cl + (1-cl)/2.; why does x_poisscl() do this?
  endif
...
function sdss_calcsigbinom, ninput_2darr, nrec_2darr, sigma=sigma, $
  ...
  if not keyword_set(cl) then cl = gauss_pdf(1.) ; = 0.841345
...
;; ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
function sdss_getcivstrct, strct_fil, dblt_name=dblt_name, dvem=dvem, $
  ...
  if keyword_set(help) then begin
     print,'Syntax - sdss_getcivstrct([strct_fil, dblt_name=, rating=, zlim=, ewlim=, '
  ...
  endelse 
  if size(civstrct_fil,/type) eq 7 then $
     substrct = xmrdfits(civstrct_fil,1,/silent) $
  ...
  if keyword_set(noBAL) then begin
     ...
     if count eq 0 then begin
        print,'sdss_getcivstrct(): no doublets not in visual BAL sightlines '
        return, -1
     endif else begin
        substrct = substrct[gd]
     endelse      
...
;           cznstr.czw_2darr[nn] = medianw([czw[gd],czw[gd]],$
;                                          [1./sigczw[gd,0]^2,1./sigczw[gd,1]^2])
;           print,ncolm_global[nn],cznstr.czw_2darr[nn],median(czw[gd],/even),ngd/total(1/czw[gd]) ;,gmean(czw[gd]) ;medianw([czw[gd],czw[gd]],[1./sigczw[gd,0]^2,1./sigczw[gd,1]^2])
           ;; range
           srt = gd[sort(czw[gd])]
...
function sdss_expandcontistrct, npix, nlin=nlin
  ;; Expand sdsscontistrct__define pixesl to allow for storage of the
  ;; stacking results
  if n_params() ne 1 then begin
...
function sdss_mkstacksumm, inp_fil, outfil=outfil, list=list, lin_fil=lin_fil, $
  ...
  ;; Write (tested before)
  if keyword_set(outfi) then begin
...
function sdss_getstackdat, stackstrct_fil, z_ion, ion, zrng=zrng, mean=mean, $
  ...
  if n_params() ne 3 then begin
     ...
     print,'                           dztol=, dwvtol=, /skip_null, count=])'
     return,-1
  endif 
  if size(stackstrct_fil,/type) eq 7 then $
  ...
  if nz_ion gt 1 then begin
     ...
     xdat = zbin
     sigxdat = rebin(xdat-stackstr[sub].zlim[0],nstack,2)
     sigxdat[*,1] = stackstr[sub].zlim[1]-xdat
     ydat = stackstr[sub].ew[mtch[0]]
     sigydat = rebin(stackstr[sub].sigew[mtch[0]],nstack,2)
  endif else begin
  ...
  srt = sort(xdat)
  ostrct = {zion:z_ion, ion:ion, mean:keyword_set(mean),$
