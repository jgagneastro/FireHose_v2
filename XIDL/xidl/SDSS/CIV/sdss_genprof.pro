...
function sdss_genprof_setnewz, vpstrct_fil, zlim, seed=seed, oseed=oseed, $
  ...
  if keyword_set(doppler) then begin
     ...
     z_new_over_old = (1. + z_sys_new) / (1. + vpstrct[index_sys].z_sys) 
     ...
     for vv=0L,nsys-1 do begin
        ...
        subvpstr.zabs = (subvpstr.zabs - subvpstr[0].z_sys) * $
                        z_new_over_old[vv] + z_sys_new[vv]
        subvpstr.z_sys = z_sys_new[vv] ; by fiat
        
        subvpstr.wvlim_sys = subvpstr.wvlim_sys * z_new_over_old[vv] ; observed frame
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
function sdss_genprof_voigt, wave, vpstrct, config_strct, nosmooth=nosmooth
  ;; Take the input structure and make the actual profile
  if n_params() ne 3 then begin 
     print,'Syntax - sdss_genprof_voigt(wave, vpstrct, config_strct)'
     return,-1
  endif 
...
function sdss_genprof_calcew, vpstrct_fil, config_fil, nosort=nosort
  ...
  for jj=0L,nsys-1 do begin
     ...
     vpfx = sdss_genprof_voigt(wave[pixmin:pixmax],subvpstr,config_strct,/nosmooth)
  ...
  if keyword_set(config_strct.ndv) then begin
     ...
     if nsub gt 0 then begin
        ...
        for ii=0L,nsub-1 do begin
           ...
           ;; Set Doppler; if locked, must come after ncolm
;           num2 = num[inum:(inum+vpstrct[ss].ncomp-1)] ;independent
           inum = inum+vpstrct[ss].ncomp ; lock with column
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
pro sdss_genprof_plot, vpstrct, config_fil, civ_fil=civ_fil
  ;; Purely generic plot of profiles
  ;; can also just instantiate EW
  if n_params() ne 2 then begin
     print,'Syntax - sdss_genprof_plot, vpstrct, config_fil, [civ_fil=]'
     return
  endif 
  clr = getcolor(/load)
  sdssdir = sdss_getsdssdir()
...
;        test = sdss_srtvpstrct(nwtmpvpstr)
     ...
     if ll eq 0 then subvpstr = nwtmpvpstr $
...
function sdss_genprof,wave, dblt_name, config_strct, cleanspec,$ 
  ...
  vpfx = sdss_genprof_voigt(wave, vpstrct, config_strct)
