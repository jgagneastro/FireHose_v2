...
;; blue
    restore, '$MIKE_PAP/Extract/Sens_mb0020.idl'
    objstr = xmrdfits('$MIKE_PAP/Extract/Obj_mb0006.fits' $
                      , 1, STRUCTYP='mikeobjstrct', /silent)
    ordrstr = xmrdfits('$MIKE_PAP/Flats/OSTr_B_01.fits',1)
    idx = where(mike.frame eq '0006' and mike.side eq 1)
    
    full_rtio_arr = fltarr(n_elements(objstr),2048) ; objstr[*].npix)
    gpx_arr = intarr(n_elements(objstr),2048) 
    ...
    if (hcopy eq 1) then x_psopen, 'b_sfnc_fsr_ang.ps', /maxs, /landscape
    ...
    plot, (objstr[jj].wave[gpx_arr[jj,0:objstr[jj].npix-1]] - centw) $ ;* objstr[jj].order/90. $
          ...
          , color= colors.black $
          , xrange = ([low,hiw]-centw)*1.75, xstyle = 1 $ ; *objstr[jj].order/90., xstyle= 1 $
          , yrange = yrng, ystyle=1 $
          , xtitle = xttl, ytitle= yttl
    xyouts, [-48], [0.082], 'Blue side', color = colors.black
    
    norm = objstr[jj].wave[gpx_arr[jj,peakn[jj]]] * objstr[jj].order 
    
    i = 10+2*34
    for jj=1L,n_elements(objstr)-2 do begin
        i = i-2
        n = peakn[jj]
        nlo = peakn[jj+1]
        nhi = peakn[jj-1]
