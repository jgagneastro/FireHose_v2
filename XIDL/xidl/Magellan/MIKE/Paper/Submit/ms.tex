...
MIKE data (recently generalized to other echelle spectrometers; 
Keck/HIRES, VLT/UVES).
This includes novel techniques for flat-fielding, wavelength calibration,
and the extraction of echelle spectorscopy.
...
Portions of this manusciprt also generalize to many of the low
...
a more complete description {\bf (http://MIKE)}.
...
with other echelle spectrometers (e.g.\ Keck/HIRES).
...
or simple IDL scripts.  The obsever is required to identify the
...
the MIKE spectrometer.  At comission, a {\bf XXX} chip with
relatively poor quantum efficiency (QE) at $\lambda < 3800$\AA\ was
installed.  This CCD was replaced on May 6, 2004 by a {\bf XXX}
device which has exceptional QE down to the atmospheric cutoff.
...
$\pm 1$ by the average order width.  {\bf [Need Figure]}
...
{\bf [If we know why MIKE fluxes better than your average bear, say
why here.]}
...
The PCA analysis provides a relatively robust means for performing
this extrapolation.}
\label{fig:trace}
\end{figure}
...
the 1st order blaze for each echelle order (ranging from number {\bf XXX to
...
\plotone{f17.eps}
\caption{{\bf RAB: Blue blaze function}
}
\label{fig:blaze}
\end{figure}
...
\plotone{f18.eps}
\caption{{\bf RAB: Blue sensitivity}
}
