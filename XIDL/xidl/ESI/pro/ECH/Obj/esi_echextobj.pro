...
; Open Slit file
  ...
  ;; explicitly mask pixels at top of order 6 (reddest order) 
  ;; which cause problems. This is a hack put in because the
  ;; waveimg is screwy and discontinuous
...
;      if not keyword_set(NOBJ) then 
     ...
     ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
      ...
      print,  '   order#   S/N' 
      forprint, 15L-ordrs[srt_s2n], sqrt(ordr_s2n[srt_s2n, 0]) $
          ...
          FOR isrt = 0L, nobj-1L DO BEGIN
              ...
              IF ordr_s2n[iorder, iobj] LE LOWSNR AND nother GT 2 THEN BEGIN
                  ...
                  IF isrt EQ 0 THEN BEGIN
                      ...
                      splog, '*************************************************'
                      wset, 1
                      plot, 15L-ordrs[other_orders], fwhm_here[other_orders] $
                      ...
                      IF nfit GT 0 THEN oplot, 15L-ordrs[other_fit] $
                        ...
                        , col = clr.cyan
                      oplot, [15L-qq], [def_fwhm], psym = 5, symsize = 2 $
                             , col = clr.green
                      oplot, 15L-ordrs, poly(15L-ordrs, fwhm_coeffs) $
                             , col = clr.red
                      wset, 0
                      sci1 = where(objstr.obj_id EQ alpha_redux[iobj] $
                                   AND objstr.order EQ qq)
