...
pro x_fitdla_initcommon, nspec, INIFIL=inifil
  ...
  if not keyword_set(INIFIL) then begin
     ...
     fd_all_fx = fltarr(200000L, nspec)
     fd_all_sig = fltarr(200000L, nspec)
     fd_all_conti = fltarr(200000L, nspec)
     fd_all_wave = dblarr(200000L, nspec)
      ...
      'IDLOUT' : begin
         if fd_nspec GT 1 then begin
             ...
             save, fd_nspec, fd_all_cstr, fd_all_npix, fd_all_conti, $
                   fd_all_fx, fd_all_sig, fd_all_wave, fd_svxymnx, $
...
;  stop
  ...
  if state.nlin NE 0 then begin
      oplot, state.wave[state.xpmnx[0]:state.xpmnx[1]], $
        ...
        state.fit[state.xpmnx[0]:state.xpmnx[1]], color=clr.darkgreen, thick=3
      ;; Mark all lines
...
;    STATE
  state = { fx: fltarr(200000L), $
            ...
            fit: fltarr(200000L) + 1., $
            conti: replicate(1.,200000L), $
            cstr: tmp2, $
            curlin: 0L, $
            nset: -1, $
            crude: 0, $  ; Crude Error
