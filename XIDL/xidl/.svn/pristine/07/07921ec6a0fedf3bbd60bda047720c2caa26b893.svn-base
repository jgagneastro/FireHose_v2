...
common hires_specplt_cmmn, hiresobj
          ...
          case ev.type of
              0 : begin ; Button press
                  case ev.press of
                      ...
                      4 : hires_specplt_SetLine, state   ; Set reference line
                      else: 
                  endcase
              end
...
;                set_value='Expecting another s !!'
          ...
          case eventch of
              ...
              ; SMOOTH
              'S': hires_specplt_smooth, state
              'U': hires_specplt_smooth, state, /reset
              ; Crude Analysis
              'E': hires_specplt_EW, state        ; Calc EW
              'N': x_specplot_Colm, state      ; Calc AODM colm
              ' ': print, 'x: '+strtrim(state.xpos,2)+$
                '   y:'+strtrim(state.ypos,2)
              ; LINES
              'L': hires_specplt_SetLine, state   ; Set reference line
              'A': begin ; Plot AlIII
              ...
              ;; ORDERS
              '0': hires_specplt_Chgordr, state, 0
              '1': hires_specplt_Chgordr, state, 1
              '2': hires_specplt_Chgordr, state, 2
              '3': hires_specplt_Chgordr, state, 3
              '4': hires_specplt_Chgordr, state, 4
              '5': hires_specplt_Chgordr, state, 5
              '6': hires_specplt_Chgordr, state, 6
              '7': hires_specplt_Chgordr, state, 7
              '8': hires_specplt_Chgordr, state, 8
              '9': hires_specplt_Chgordr, state, 9
              '=': hires_specplt_Chgordr, state, /add
              '-': hires_specplt_Chgordr, state, /sub
              ; Postscript
              'P': hires_specplt_psfile, state  
              ; QUIT
              'q': begin
                  widget_control, ev.top, /destroy
                  return
              end
              else:  print, 'hires_specplt: Not a valid key!' ; Nothing
          endcase
      end
      'DONE' : begin
          widget_control, ev.top, /destroy
...
; Update Plot
  if not keyword_set(NOUPD) then hires_specplt_UpdatePlot, state
...
pro hires_specplt_UpdatePlot, state
...
; Plot Data
  ...
  if flg_lines EQ 1 then hires_specplt_PltLines, state
  
  ; EW
  if state.flg_EW NE 0 then hires_specplt_PltEW, state
...
pro hires_specplt_Reset, state

common hires_specplt_cmmn

  case state.flg_fspec of 
      0: begin ; Box
          ;; Set wave, fx
;          a = where(hiresobj[state.ordr].box_fx NE 0., na, $
;                    COMPLEMENT=b, NCOMPLEMENT=nb)
;          amin = (a[0] > 0)
;          if amin GT 100 then amin = 0L
;          bmax = b[nb-1]
;          if bmax LT hiresobj[state.ordr].npix/2 then $
;            bmax = 4999 else bmax = a[na-1]
;          npix = bmax-amin+1
;          state.npix = npix
          amin = 0L
          bmax = hiresobj[state.ordr].nrow-1
          state.npix = bmax+1
          npix = state.npix
          
          state.wave[0:npix-1] = hiresobj[state.ordr].box_wv[amin:bmax]
          state.fx[0:npix-1] = hiresobj[state.ordr].box_fx[amin:bmax] 
          state.sig[0:npix-1] = sqrt(hiresobj[state.ordr].box_var[amin:bmax])
      end
      1: begin ; Optimal
          ;; Set wave, fx
;          a = where(hiresobj[state.ordr].wave NE 0., na, $
;                    COMPLEMENT=b, NCOMPLEMENT=nb)
...
;          endelse
          amin = 0L
          bmax = hiresobj[state.ordr].npix - 1
          npix = bmax-amin+1
          state.npix = npix
          
          state.wave[0:npix-1] = hiresobj[state.ordr].wave[amin:bmax]
          state.fx[0:npix-1] = hiresobj[state.ordr].fx[amin:bmax]
          state.sig[0:npix-1] = sqrt(hiresobj[state.ordr].var[amin:bmax] > 0.)
      end
      2: begin ; FSPEC
          ;; Set wave, fx
          ido = where(state.ordr EQ hiresobj.phys_ordr, na)
          if na EQ 0 then stop
          a = where(hiresobj.wave[*,ido] NE 0., na, $
                    COMPLEMENT=b, NCOMPLEMENT=nb)
          if na EQ 0 then begin
              amin = 0L
              bmax = 100L
...
;              if amin GT 100 then amin = 0L
          ...
          state.wave[0:npix-1] = hiresobj.wave[amin:bmax,ido]
          state.fx[0:npix-1] = hiresobj.fx[amin:bmax,ido]
          state.sig[0:npix-1] = sqrt(hiresobj.var[amin:bmax,ido] > 0.)
      end
      3: begin ; fluxed
          ;; Set wave, fx
          a = where(hiresobj[state.ordr].wave NE 0., na, $
          ...
          state.wave[0:npix-1] = hiresobj[state.ordr].wave[amin:bmax]
          state.fx[0:npix-1] = hiresobj[state.ordr].flux[amin:bmax]
          state.sig[0:npix-1] = hiresobj[state.ordr].sig[amin:bmax]
      end
      4: begin ; Sky
          ;; Set wave, fx
          a = where(hiresobj[state.ordr].sky NE 0., na, $
                    COMPLEMENT=b, NCOMPLEMENT=nb)
          amin = (a[0] > 0)
          if amin GT 100 then amin = 0L
          bmax = b[nb-1]
          if bmax LT hiresobj[state.ordr].nrow/2 then $
            bmax = 4999 else bmax = a[na-1]
          npix = bmax-amin+1
          state.npix = npix
          
          state.wave[0:npix-1] = hiresobj[state.ordr].sky_wv[amin:bmax]
          state.fx[0:npix-1] = hiresobj[state.ordr].sky[amin:bmax] 
...
pro hires_specplt_SetLine, state
...
pro hires_specplt_PltLines, state, IMG=img
...
pro hires_specplt_Colm, state

common x_specplot_lines

  ; Check for error array
  if state.flg_sig NE 1 then begin
      print, 'hires_specplt_Colm: Need to specify the error array!'
      return
  endif
...
pro hires_specplt_guess, state, val, IMG=img
...
pro hires_specplt_psfile, state
...
; Device
  ...
  state.psfile = 1
  hires_specplt_UpdatePlot, state
...
pro hires_specplt_smooth, state, RESET=reset
...
pro hires_specplt_Chgordr, state, newordr, ADD=add, SUB=sub
...
common hires_specplt_cmmn
  ...
  if keyword_set( ADD ) or keyword_set( SUB ) then begin
      if keyword_set( ADD ) then begin
          ...
          if state.flg_fspec NE 2 then begin
              if newordr EQ n_elements(hiresobj) then newordr = 0
          endif else begin
              if hiresobj.phys_ordr[newordr] LE 0 then begin
                  pos = where(hiresobj.phys_ordr GT 0, npos)
                  newordr = pos[0]
              endif
          endelse
          state.ordr = newordr
      ...
      if keyword_set( SUB ) then begin
          newordr = state.ordr - 1
          if state.flg_fspec NE 2 then begin
              if newordr LT 0 then newordr = n_elements(hiresobj)-1
          endif else begin
              if hiresobj.phys_ordr[newordr] LE 0 then begin
                  pos = where(hiresobj.phys_ordr GT 0, npos)
                  newordr = pos[npos-1]
  ...
  case state.flg_fspec of
      0: state.phys_ordr = hiresobj[state.ordr].order 
      1: state.phys_ordr = hiresobj[state.ordr].order 
      2: state.phys_ordr = hiresobj.phys_ordr[state.ordr]
      3: state.phys_ordr = hiresobj[state.ordr].order 
      4: state.phys_ordr = hiresobj[state.ordr].order 
      else: stop
  endcase

  ;; RESET
  widget_control, state.ordr_id, set_value=state.phys_ordr
  hires_specplt_Reset, state
...
pro hires_specplt, spec_fil, obj_id, XSIZE=xsize, YSIZE=ysize, $
...
;
;  print,'Syntax - ' + $
;    'hires_specplt, spec_fil, obj_id, XSIZE=,YSIZE=, /NOCTB, WAVE=, '
...
; Input the Obj structure
  if not keyword_set( FSPEC ) then begin
      ...
      hires_wrfspec, objstr, spec_fil, /read, UVES=uves 
      gdobj = [0L]
      ;; Initial order
...
; Init common
  hires_specplt_initcommon
  hiresobj = objstr[gdobj]
  delvarx, objstr
...
; PHYSORDR
  if not keyword_set( FSPEC ) then state.phys_ordr = hiresobj[state.ordr].order $
    else state.phys_ordr = hiresobj.phys_ordr[state.ordr]
...
;    WIDGET
  base = WIDGET_BASE( title = 'hires_specplt', /column, $
...
; Send to the xmanager
  if not keyword_set(BLOCK) then xmanager, 'hires_specplt', base, /no_block $
  else xmanager, 'hires_specplt', base
