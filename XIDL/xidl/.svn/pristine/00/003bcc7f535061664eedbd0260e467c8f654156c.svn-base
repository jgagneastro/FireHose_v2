...
;
; OPTIONAL KEYWORDS:
;  /NOCLM -- Don't bother to do column densities.  Useful for
;            plotting only
;
; OPTIONAL OUTPUTS:
;
...
pro lls_allabnd, lls, nn, sys, FLG_PLT=flg_plt, TYP_FLG=typ_flg, ROOT=root, $
                 NOCLM=noclm, LUN=lun, FLGNN=flgnn
...
;  resolve, 'ew_to_colm'
  ...
  ;; MIKEB
  if lls[nn].systems[sys].ndfil MOD 32 GT 8 then begin
      readf, lun, dumc
      lls[nn].systems[sys].MBfil = dumc
  endif
  ;; MIKER
  if lls[nn].systems[sys].ndfil MOD 64 GT 16 then begin
      readf, lun, dumc
...
;      if lambda EQ 1335.7077d then ion = 6
      ...
      ;; SiII*
      if min(abs(si2wv-lambda)) LT 1e-3 then ion = 6
      ...
      if flg_fil GT 128 then iflg=2 else iflg=1
      if mdflg NE SV_FIL then begin
          case mdflg of
              1: dat = x_readspec(lls[nn].systems[sys].Hfil, /struct, /autofsig)
              2: dat = x_readspec(lls[nn].systems[sys].Efil, /struct, /autofsig)
              4: dat = x_readspec(lls[nn].systems[sys].Ufil, /struct, /autofsig, INFLG =iflg)
              8: dat = x_readspec(lls[nn].systems[sys].Xfil, /struct, /autofsig)
              16: dat = x_readspec(lls[nn].systems[sys].MBfil, /struct, /autofsig)
              32: dat = x_readspec(lls[nn].systems[sys].MRfil, /struct, /autofsig)
              else: stop
          endcase
      ...
      if flg_sat GT 0 OR ionflg EQ 2 OR ionflg EQ 3  then begin
          ...
          if (ionflg MOD 8) gt 1 then begin ; Keep flg given by hand
              lls[nn].systems[sys].ion[atnmb].state[ion,idx].flgclm = ionflg
              print, 'lls_allabnd: Taking value in the file'
