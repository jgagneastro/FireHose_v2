...
;   /TRIM_FFT - Demand same array size in x_crossarc for FFT
;   /FEW_LINES - Arc has very few lines to fit (e.g. bad dewar4
;                Hamspec data)
;
; OPTIONAL OUTPUTS:
;
...
pro x_fitarc_ps, svdecomp, filnm, WV=wv, FIT=fit, REJPT=rejpt,$
  ...
  xyouts, xlbl, 0.6, 'Order = '+string(ordr, FORMAT='(i3)')+$
    '   !9Dl!X = '+string(dwv, FORMAT='(f6.4)'), charsize=1.3
  xyouts, xlbl, 0.48, 'RMS='+string(rms, FORMAT='(f6.4)')+$
...
function x_fitarc, arc_fil, ordr_str, out_fil, $
                   ...
                   GUESSARC=guessarc, SATUR=satur, STRETCH=stretch, $
                   ...
                   FGAUSS=fgauss, MEDWID=medwid, MXORDER=mxorder, $
                   TRIM_FFT=trim_fft, FEW_LINES=few_lines
...
;
  ...
  if not keyword_set( MXORDER ) then mxorder = 50L
  ...
  if flg_ordr EQ 0 then begin
      sv_aspec = fltarr(sz_arc[1], MXORDER)
      fittmp = { fitstrct }
      all_arcfit = replicate(fittmp, MXORDER)
      rejtmp = { $
                 ngdf: 0L, $
                 gdfpt: lonarr(100), $
                 gdfpx: dblarr(100), $
      ...
      rejstr = replicate(rejtmp, MXORDER)
      lintmp = { $
                 pix: dblarr(90), $
                 wv: dblarr(90), $
                 nlin: 0 $
               }
      sv_lines = replicate(lintmp, MXORDER)
...
; AUTO or SEMI-AUTO (PINTER)
          ...
          ;; Check to see if this order was well done in the archive
          if guess_fit[igss].nord LE 0 then begin   ; Set first fit to 3
              print, 'x_fitarc:  Not fit in guess spectra skip to next order'
              fit_msk[ii] = -1B
              continue
           endif
...
;          step = lindgen(80L) - 40L + row_shift
          ...
          sv_aspec_size = size(sv_aspec[*,ii],/dim)
          new_size = sv_aspec_size[0] + stretch
          resz_sv_aspec = dblarr(new_size)
          resz_sv_aspec[0:sv_aspec_size[0]-1] = sv_aspec[*,ii]
          resz_guess_spec = congrid(guess_spec[*,igss],new_size,/interp)

          corr = c_correlate((resz_sv_aspec<1000.), $
                             (resz_guess_spec<1000.), step, /double)
          mx = max(corr, imx)
          imx = step[imx]
          shft = -imx 
...
;              npix = n_elements(sv_aspec[*,ii])
             ...
             ;if jj EQ 135 then debug2 = 1 else debug2 = 0
              x_templarc, sv_aspec[*,ii], lines, guess_fit[igss], MSK=msk, $
                          ...
                          toler=2.,STRETCH=stretch, DEBUG=debug2
              gdfit = where(lines.flg_plt EQ 1, ngd)
              ;if jj EQ 76 then stop
              ;if jj EQ 76 then begin
              ;   x_templarc, sv_aspec[*,ii], lines3, guess_fit[igss], MSK=msk, $
              ;              SHFT=shft, PKWDTH=pkwdth, FORDR=9, $
              ;              PKSIG=psig, FLG=flg_templ, toler=2.
              ;  x_templarc, sv_aspec[*,ii], lines2, guess_fit[igss], fordr=9, $
              ;              SHFT=shft, PKWDTH=3, toler=2., $
              ;              PKSIG=psig, FLG=flg_templ;, BEST=best, FWEIGHT=fweight, FGAUSS=fgauss
              ;  stop
              ; endif
              if flg_templ EQ -1 then begin
                  print, 'x_fitarc: Insufficient lines for AUTO!! flg_templ'
                  continue
              endif
          endelse
          ...
          if ngd LE 4 then begin
              if jj GT 46 AND jj LT 103 and not keyword_set(FEW_LINES) then begin
                  print, 'x_fitarc: Insufficient lines for AUTO!! n<4'
                  stop
                  continue
              endif else begin
                  if ngd LE 1 then begin
                      print, 'x_fitarc: Insufficient lines for AUTO!! n<1'
                      fit_msk[ii] = -1
;                      stop
                      continue
                  endif else begin
...
;                  continue
          ...
          x_templarc, sv_aspec[*,ii], lines, tmp_fit, MSK=msk, PKWDTH=pkwdth, $
            FORDR=9, SHFT=shft, /LOG, PKSIG=fsig, FWEIGHT=fweight, FGAUSS=fgauss;, STRETCH=stretch
	  ...
	  if ngd GT 0 then begin
          ...
          if not keyword_set( PINTER ) then begin ;; AUTO FIT
              if ngd EQ 0 then begin
                  ...
                  splog, 'Stop removed by Moustakas!'
;                 stop
              endif else begin
                  if fin_fit.nord EQ 0 then begin
                     sv_logwv[*,ii] = 0.
                     print, 'x_fitarc: Skipping order ', jj, ' all but one line rejected'
                     rejstr[ii].ngdf = 0
                     continue
                  endif 
                  fit = x_fitrej(lines[gdfit].pix, alog10(lines[gdfit].wave), $
...
;  restore, out_fil
  if keyword_set( QAFIL ) then begin
      ...
      for ii=0L,nordr-1 do begin
          ...
          dwv = abs(median(wv - shift(wv,1)))
          ;; Subroutine 
;          print,order_str[ii].order,rejstr[ii].ngdf,all_arcfit[ii].nord
;          stop
