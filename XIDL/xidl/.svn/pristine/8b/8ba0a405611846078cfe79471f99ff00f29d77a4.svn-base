...
; y8
  ...
  if state.flg_leg then begin
     ...
     LEGEND, state.legend, psym=psym, colors=colors, outline_color=color.black, textcolors=colors, /top_legend, /center_legend
  endif
...
;  if not keyword_set( XSIZE ) then    xsize = ssz[0]-200
  ...
  if not keyword_set( COLOR5 ) then   color5 = color.green
  if not keyword_set( COLOR6 ) then   color6 = color.magenta
  if not keyword_set( COLOR7 ) then   color7 = color.gray
  if not keyword_set( COLOR8 ) then   color8 = color.gray
