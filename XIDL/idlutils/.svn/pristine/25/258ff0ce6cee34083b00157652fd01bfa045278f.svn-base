...
/* Modified to handle varying number of fibers per block, and varying extraction size for MaNGA survey */
/* Dec 2012 - Brian Cherinka, Toronto */

IDL_LONG ml_trace_crude
  (int      argc,
   void *   argv[])
{
   IDL_LONG    nx;
   ...
   /*radius = *((float *)argv[4]);*/
   radius = ((float *)argv[4]);	 /*array of ntrace elements, like xstart*/
   ...
   for (itrace=0; itrace < ntrace; itrace ++) {
      ...
      iy = ystart[itrace];
      xinit = xstart[itrace];
      recenter_fweight(nx, fimage[iy], invvar[iy], radius[itrace], xinit,
       &xfit, &xfiterr);
