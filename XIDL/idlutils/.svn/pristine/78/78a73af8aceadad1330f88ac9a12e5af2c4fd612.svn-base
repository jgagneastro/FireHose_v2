...
;-
;------------------------------------------------------------------------------
function ucac3_intersect, dec0, node, incl, nu
...
;------------------------------------------------------------------------------
function ucac3_readgc_add, outdat, thiszone, ravec

   if (ravec[0] GT 360) then ravec = ravec - 360
   newdat = ucac3_readzone(thiszone, ravec[0], ravec[1])
...
function ucac3_readcircle1, racen, deccen, radius, thiszone, decmin, decmax

   if ((decmax LT deccen-radius) OR (decmin GT deccen+radius)) then return, 0

   DRADEG = 180.d0 / !dpi
   decvec= [decmin, decmax, deccen]
   maxdec= max(abs(decvec), imaxdec)
   decuse= decvec[imaxdec]
   ravec = racen + [-1,1] * radius / cos(decuse/DRADEG)
   ...
   if (ravec[1] - ravec[0] GT 360) then begin
      ...
      outdat = ucac3_readgc_add(outdat, thiszone, [0,ravec[1]-360])
   endif else begin
      outdat = ucac3_readgc_add(outdat, thiszone, ravec)
   endelse
...
;------------------------------------------------------------------------------
; Note that DECMIN,DECMAX is the declination range of zone number THISZONE.
function ucac3_readgc1, node, incl, hwidth, thiszone, decmin, decmax
   ...
   ; CASE: Take a cap at the top of the great circle
      ...
      nu = incl - dec1max
      ravec = ucac3_intersect(decmin, node, incl, nu)
   ...
   ; CASE: Take a cap at the bottom of the great circle
      ...
      nu = incl - dec2max
      ravec = ucac3_intersect(decmax, node, incl, nu)
      if (nu GT 0) then ravec = reverse(ravec)

   ; CASE: Take two intersections...
   endif else begin
      ravec1 = ucac3_intersect(decmax, node, incl, dec1max-incl)
      ravec2 = ucac3_intersect(decmin, node, incl, dec2max-incl)
   ...
   if (ravec[0] LT ravec[1]) then begin
      outdat = ucac3_readgc_add(outdat, thiszone, ravec[0:1])
   endif else begin
      outdat = ucac3_readgc_add(outdat, thiszone, [ravec[0], 360])
      outdat = ucac3_readgc_add(outdat, thiszone, [0, ravec[1]])
   endelse
   if (n_elements(ravec) GT 2) then begin
      if (ravec[2] LT ravec[3]) then begin
         outdat = ucac3_readgc_add(outdat, thiszone, ravec[2:3])
      endif else begin
         outdat = ucac3_readgc_add(outdat, thiszone, [ravec[2], 360])
         outdat = ucac3_readgc_add(outdat, thiszone, [0, ravec[3]])
      endelse
   endif
...
;------------------------------------------------------------------------------
function ucac3_read, racen=racen, deccen=deccen, radius=radius, $
 node=node1, incl=incl1, hwidth=hwidth, decrange=decrange1
   ...
   for thiszone=min(uindex.zn), max(uindex.zn) do begin
      ...
      if (ct GT 0) then begin
         ...
         if (decmax GE decrange[0] AND decmin LE decrange[1]) then begin
            if (qreadstripe) then begin
               moredat = ucac3_readgc1(node, incl, hwidth, thiszone, $
                decmin, decmax)
            endif else begin
               moredat = ucac3_readcircle1(racen, deccen, radius, thiszone, $
                decmin, decmax)
            endelse
            if (keyword_set(moredat)) then begin
   ...
   if (keyword_set(outdat)) then begin
      ...
      if (qreadcircle) then begin
         adiff = djs_diff_angle(outdat.ra, outdat.dec, racen, deccen)
         qkeep = qkeep AND (adiff LE radius)
      endif
      if (qreadstripe) then begin
         radec_to_munu, outdat.ra, outdat.dec, node=node, incl=incl, $
          mu, nu
   ...
   if (keyword_set(decrange1) AND keyword_set(outdat)) then begin
      ikeep = where(outdat.dec GE decrange[0] $
       AND outdat.dec LE decrange[1], nkeep)
      if (nkeep EQ 0) then outdat = 0 $
       else outdat = outdat[ikeep]
   endif
