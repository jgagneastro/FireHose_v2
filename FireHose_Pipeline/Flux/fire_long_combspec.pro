...
PRO FIRE_LONG_COMBSPEC, influx, inivar, loglam $
                   ...
                   , USER_WEIGHTS=user_weights, snr_out=snr_out
...
;;;;;;;;;;;;;;;;;;
; Calculate the SNR^2 for weighting purposes
;;;;;;;;;;;;;;;;;
...
ENDIF else begin
   avg_sn = sqrt(total(user_weights)/double(nimgs))
ENDELSE

IF (avg_sn LE 4.0D AND not(keyword_set(USER_WEIGHTS))) THEN BEGIN
    sn2_weights = (replicate(1.0D, nspec) # mean_sn2)
    snr_out=sqrt(mean_sn2)
    splog, 'Low S/N regime. Using one (S/N)^2 weight per spectrum'
ENDIF 
snr_out=sqrt(mean_sn2)

IF (keyword_set(USER_WEIGHTS)) then begin
    sn2_weights = (replicate(1.0D, nspec) # user_weights)
    splog, 'User-specified weightings employed in spectral combine.'
    for j=0, nimgs-1 do begin
       print, "Frame "+strtrim(j,2)+", Weight = "+strtrim(user_weights[j],2)
    endfor
ENDIF
