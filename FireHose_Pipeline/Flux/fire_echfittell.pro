...
; CALLING SEQUENCE:
;   
;   fit = mage_echfittell([xdat],ydat,func=,nord=, /inter, xsize=, ysize=)
;
; INPUTS:
...
;
; EXAMPLES:
;   fit = fire_echfittell(x, y, 'POLY', nord=5, /inter)
;
;
...
pro fire_echfittell_initcommon, inord, fitstr

;

common fire_echfittell_fit, nord, tot_fit, sens_func

  nord = inord
  if not keyword_set( FITSTR ) then $
    fitstr = x_setfitstrct(FUNC='LEGEND', NORD=15L, LSIG=3., HSIG=3., $
                           FLGREJ=1)
...
;function fire_echfittell_ev, ev
pro fire_echfittell_event, ev

common fire_echfittell_fit 
  ...
  case uval of
      ...
      'FUNCLIST' : begin
          ...
          ; Update fit
          fire_echfittell_UpdateFit, state
      end
      'UP' : begin
          ...
          case ev.type of
              0 : begin ; Button press
                  case ev.press of
                      ...
                      4 : fire_echfittell_setordr, state
                      else :
                  endcase
              end
      ...
      'TEXT' : begin
          ...
          if (state.flg_zero EQ 1 AND eventch NE 's') then begin
              print, 'fire_echfittell: Expecting another s or LMB click !!'
              WIDGET_CONTROL, state.base_id, set_uvalue = state, /no_copy
              return
           endif
           if (state.flg_unzero EQ 1 AND eventch NE 'x') then begin
              print, 'fire_echfittell: Expecting another x'
              WIDGET_CONTROL, state.base_id, set_uvalue = state, /no_copy
              return
           endif
          case eventch of
              'u': begin
                  ...
                  print, 'fire_echfittell: Fit order = ', tot_fit[gdo].nord
;                  widget_control, state.lblordr_id, $
;                    set_value=string(fitprm.nord, format='(i4)')
                  fire_echfittell_UpdateFit, state, ordr=state.currordr
              end
              'd': begin
                  if state.currordr LT 1 then return
                  gdo = where(state.objstr.order EQ state.currordr)
                  tot_fit[gdo].nord = (tot_fit[gdo].nord - 1) > 1
                  print, 'fire_echfittell: Fit order = ', tot_fit[gdo].nord
;                  widget_control, state.lblordr_id, $
;                    set_value=string(fitprm.nord, format='(i4)')
                  fire_echfittell_UpdateFit, state, ordr=state.currordr
...
;              end
              ...
              'x': begin        ; Region
                  fire_echfittell_unzeroreg, state
              ...
              'f': begin        ; Region
                  fire_echfittell_forcereg, state
...
;              'D': fire_echfittell_Delete, state ; Delete/Undelete a data point
;              'x': fire_echfittell_DelReg, state        ; Delete one region
;              'S': fire_echfittell_DelReg, state, /all  ; Delete all regions and reset
...
;              'V': fire_echfittell_SetValues, state     ; Plot values
;              'C': fire_echfittell_ClearRej, state ; Clear all rejected points
              'H': x_helpwidg, state.help
              'q': begin
;                  fire_echfittell_setpnt, state
...
;            REJECTION
      'HSIGVAL': begin
          ...
          fire_echfittell_UnDelRej, state
...
; Update Plot
  fire_echfittell_UpdatePlot, state
...
pro fire_echfittell_UpdatePlot, state, ALL=all
...
common fire_echfittell_fit
  ...
  gdw = where( state.objstr.wave GT state.xymnx[0] AND $
               state.objstr.wave LT state.xymnx[2], ngdw )
  ...
  gd2 = where( (state.objstr.wave)[gdw] GT 0. $
  ...
  if state.currordr GT 0 then begin
      gdo = where( (state.objstr.wave)[gdw] GT 0. $
                   and (state.objstr.sig)[gdw] GT 0. $
 ...
 ;;  plot, (state.objstr.wave)[gd1], (state.objstr.flux)[gd1], $
;;     xrange=[state.xymnx[0],state.xymnx[2]], $
;;     yrange=[state.xymnx[1],state.xymnx[3]], pos=state.pos, $
;;     charsize=1.2, psym=1, background=clr.white, color=clr.black, $
;;     xtitle='!17Wavelength', xmargin=[0,0], ymargin=[0,0], xstyle=1,$
;;     ystyle=1, /nodata
  ...
  ;; Plot the good
  IF ngd1 GT 0 THEN $
     oplot, (state.objstr.wave)[gd1], (state.objstr.flux)[gd1], psym = 1 $
            , color = clr.red
  
  IF ngd2 GT 0 THEN $
     oplot, (state.objstr.wave)[gd2], (state.objstr.flux)[gd2], psym = 1 $
            , color = clr.blue

  IF state.currordr GT 0 then BEGIN
      oplot, (state.objstr.wave)[gdo], (state.objstr.flux)[gdo], $
             psym = 1, color = clr.black
      oplot, (state.objstr.wave)[gdo], (state.fit)[gdo], $
             color = clr.magenta, thick = 3.0
  END

  ;; Plot the bad
  IF nbad GT 0 THEN $
     oplot, (state.objstr.wave)[bad], (state.objstr.flux)[bad], psym = 1 $
            , color = clr.green
  ;; Plot the force
  IF nforce GT 0 THEN $
     oplot, (state.objstr.wave)[force], (state.objstr.flux)[force], psym = 1 $
            , color = clr.yellow
  ...
  IF ngd1 GT 0 THEN oplot, (state.objstr.wave)[gd1], (state.resid1)[gd1] $
                           , psym = 1, color = clr.red
  IF ngd2 GT 0 THEN oplot, (state.objstr.wave)[gd2], (state.resid1)[gd2] $
                           , psym = 1, color = clr.blue
  if state.currordr GT 0 then $
     oplot, (state.objstr.wave)[gdo], (state.resid1)[gdo] $
            , psym = 1, color = clr.black
  ...
  plot, [0.], [0.], xrange=[state.xymnx[0],state.xymnx[2]], $
    ...
    charsize=1.2, background=clr.white, color=clr.black, $
    xmargin=[8,0], ymargin=[0,0], xstyle=1, ystyle=1, /nodata
  IF ngd1 GT 0 THEN oplot, (state.objstr.wave)[gd1], (state.newflux)[gd1] $
                           , psym = 1, color = clr.red
  IF ngd2 GT 0 THEN oplot, (state.objstr.wave)[gd2], (state.newflux)[gd2] $
                           , psym = 1, color = clr.blue
;  if state.currordr GT 0 then $
;    oplot, (state.objstr.wave)[gdo], alog10((state.newflux)[gdo]), $
;    psym=1, color=clr.black
...
pro fire_echfittell_UpdateFit, state, ALL=all, ORDR=ordr
...
common fire_echfittell_fit
  ...
  for qq=iord,ford do begin
      ...
      fit = x_fitrej(state.objstr[qq].wave[gd], $
                     state.objstr[qq].flux[gd], $
                     sig = sig_fit[gd], $
                     fitstr=tmp, rejpt=rejpt)
;                     state.objstr[qq].flux[gd], $
;                     sig=sqrt(state.objstr[qq].var[gd]>0), $

      if rejpt[0] NE -1 then state.objstr[qq].sig[gd[rejpt]] = 0.
      tot_fit[qq] = tmp

      ;; Residuals
      allf = x_calcfit(state.objstr[qq].wave, fitstr=tmp)
      state.resid1[*,qq] = state.objstr[qq].flux - allf

      ;; Std flux
      state.newflux[*,qq] = state.objstr[qq].flux / allf
      state.fit[*, qq] = allf
...
;  Set order
;;;;;;;;;;;;;;;;;;;;
pro fire_echfittell_setordr, state

  ;; Grab x position
  xv = xgetx_plt(state,/strct)

  ;; Grab closest pixel
  mn = min(abs(state.objstr.wave-xv),imn)
  dwv = abs((state.objstr.wave)[imn] - (state.objstr.wave)[imn+1])

  ;; Grab closest within 10pixels
  gd = where(abs(state.objstr.wave-xv) LT 10*dwv)
...
pro fire_echfittell_Reset, state

  ;; Set the flux
  for qq=0L,n_elements(state.objstr)-1 do begin
      ;; gd
      gd = where(state.objstr[qq].wave GT 0. AND state.objstr[qq].flux GT 1.D-8)
      linterp, state.std_wv, state.std_fx, state.objstr[qq].wave[gd], fx
      state.objstr[qq].flux[gd] = state.objstr[qq].flux[gd] / fx
      state.objstr[qq].sig[gd] = sqrt(state.objstr[qq].var[gd]) / fx
  endfor
  state.svsig = state.objstr.sig

  ;; Good ones
  gd = where(state.objstr.wave GT 0. and state.svsig GT 0. AND state.objstr.flux GT 1.D-8)

  ;; Set xdat, ydat
  xdat = (state.objstr.wave)[gd]
...
pro fire_echfittell_zeroreg, state

  ;; Check current order
  if state.currordr LT 1 then begin
      print, 'fire_echfittell:  Need to set the order with RMB first!'
      return
  endif
  ...
  if state.flg_zero MOD 2 EQ 0 then begin
      ...
      ;; Find the pixels
      gdo = where(state.objstr.order EQ state.currordr)
      zpix = where(state.objstr[gdo].wave GT state.zero[0] AND $
                   state.objstr[gdo].wave LT state.zero[1], nzp)
      if nzp NE 0 then $
...
pro fire_echfittell_unzeroreg, state
  ...
  if state.flg_unzero MOD 2 EQ 0 then begin
      ...
      ;; Find the pixels
      gdo = where(state.objstr.order EQ state.currordr)
      zpix = where(state.objstr[gdo].wave GT state.unzero[0] AND $
                   state.objstr[gdo].wave LT state.unzero[1], nzp)
      if nzp NE 0 then $
...
; This routine added by JFH to unzero regions  
pro fire_echfittell_forcereg, state
  ...
  if state.flg_force MOD 2 EQ 0 then begin
      ...
      ;; Find the pixels
      gdo = where(state.objstr.order EQ state.currordr)
      zpix = where(state.objstr[gdo].wave GT state.force[0] AND $
                   state.objstr[gdo].wave LT state.force[1], nzp)
      if nzp NE 0 then $
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;  Reset SVXY
pro fire_echfittell_UpdateSVXY, state
  ; Set xdat, ydat
...
pro fire_echfittell, objstr, std_wv, std_fx, outfil, XSIZE=xsize, FITSTR=fitstr,$
...
;
  if  N_params() LT 4  then begin 
    print,'Syntax - ' + $
      'fit = fire_echfittell(objstr, std_wv, std_fx, outfil, ' + $
...
;  readcol, stdfil, swv, sfx
  ...
  fire_echfittell_initcommon, n_elements(objstr), fitstr
  sens_func = objstr.flux
  sens_func[*] = 0.
  ...
  state = { $
            ...
            flg_force: 0, $
            force: fltarr(2), $
            ordnum: replicate(1., n_elements(objstr[0].wave)) # objstr.order, $
            currordr: 0L, $
            svxymnx: fltarr(4), $
            xymnx: fltarr(4), $
            tmpxy: fltarr(4), $
  ...
  ;;    WIDGET
  base = WIDGET_BASE( title = 'fire_echfittell: Interactive ' + $
...
; Realize
  ...
  ;; Update
  fire_echfittell_Reset, state
  fire_echfittell_UpdateFit, state, /all
  fire_echfittell_UpdatePlot, state, /all
  WIDGET_CONTROL, base, set_uvalue = state, /no_copy
  
  xmanager, 'fire_echfittell', base
