...
;-
;------------------------------------------------------------------------------
pro fire_proc, filename, flux, invvar, hdr = hdr $
  ...
  if  N_params() LT 2  then begin 
      print,'Syntax - ' + $
        'fire_proc, fil, flux, [invvar], HDR=, BIASFILE=, GAIN= [v1.0]' 
      return
  endif 
...
if (arg_present(invvar)) then $
  fire_oscan, filename, flux, invvar, hdr = hdr, bin=bin, verbose=verbose $
              , rnoise = rnoise, gain = gain $
else $
   fire_oscan, filename, flux, hdr = hdr, verbose = verbose, bin = bin $
               , rnoise = rnoise, gain = gain
