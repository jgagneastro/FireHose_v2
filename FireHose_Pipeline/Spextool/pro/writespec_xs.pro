...
;    
; PURPOSE:
;     Writes extended source spectra to disk.
;
; CATEGORY:
;     Spectroscopy
;    
; CALLING SEQUENCE:
;     writespec_xs,spectra,fullpath,aimage,sky,flat,naps,orders,start,stop,$
;                  hdrinfo,appos,apradius,mode,slith_pix,slith_arc,$
;                  slitw_pix,slitw_arc,xunits,yunits,xtitle,ytitle,$
;                  version,BGR=bgr,BGORDER=bgorder,TRACEFILE=tracefile,$
;                  WAVECAL=wavecal,DISP=disp,FITS=fits,TEXT=text,RMS=rms,$
;                  RES=res,LINEARITY=linearity,EXT2D=ext2d,CANCEL=cancel
;    
; INPUTS:
...
;     appos     - An array of aperture positions
...
;     version   - Spextool version
;    
; OPTIONAL INPUTS:
;     None
;
; KEYWORD PARAMETERS:
;     BGR       - The background regions of the extraction 
;                 (see extractspec_xs.pro)
...
;     WAVECAL   - A string giving the name of the wavecal      
...
;     2003-04-10 - Added the appos input
;     2005-07-04 - Modified to accept new hdrinfo structure
...
;-
pro writespec_xs,spectra,fullpath,aimage,sky,flat,naps,orders,start,stop,$
                 hdrinfo,appos,apradius,mode,slith_pix,slith_arc,$
                 slitw_pix,slitw_arc,xunits,yunits,xtitle,ytitle, $
                 version,BGR=bgr,BGORDER=bgorder,TRACEFILE=tracefile, $
                 WAVECAL=wavecal,DISP=disp,FITS=fits,TEXT=text,RMS=rms, $
...
cancel = cpar('writespec_xs',spectra,1,'Spectra',[2,3,4,5],[2,3])
if cancel then return
cancel = cpar('writespec_xs',fullpath,2,'Fullpath',7,0) 
if cancel then return
cancel = cpar('writespec_xs',aimage,3,'Aimage',7,0) 
if cancel then return
cancel = cpar('writespec_xs',sky,4,'Sky',7,0)
if cancel then return
cancel = cpar('writespec_xs',flat,5,'Flat',7,0)
if cancel then return
cancel = cpar('writespec_xs',naps,6,'Naps',[2,3,4,5],0) 
if cancel then return
cancel = cpar('writespec_xs',orders,7,'Orders',[2,3,4,5],[0,1]) 
if cancel then return
cancel = cpar('writespec_xs',start,8,'Start',[2,3,4,5],0)
if cancel then return
cancel = cpar('writespec_xs',stop,9,'Stop',[2,3,4,5],0) 
if cancel then return
cancel = cpar('writespec_xs',hdrinfo,10,'Hdrinfo',8,[0,1])
if cancel then return
cancel = cpar('writespec_xs',appos,11,'Appos',[2,3,4,5],[1,2]) 
if cancel then return
cancel = cpar('writespec_xs',apradius,12,'Apradius',[2,3,4,5],[0,1]) 
if cancel then return
cancel = cpar('writespec_xs',mode,13,'Mode',7,0)
if cancel then return
cancel = cpar('writespec_xs',slith_pix,14,'Slith_pix',[2,3,4,5],0)
if cancel then return
cancel = cpar('writespec_xs',slith_arc,15,'Slith_arc',[2,3,4,5],0)
if cancel then return
cancel = cpar('writespec_xs',slitw_pix,16,'Slitw_pix',[2,3,4,5],0)
if cancel then return
cancel = cpar('writespec_xs',slitw_arc,17,'Slitw_arc',[2,3,4,5],0)
if cancel then return
cancel = cpar('writespec_xs',xunits,18,'Xunits',7,0)
if cancel then return
cancel = cpar('writespec_xs',yunits,19,'Yunits',7,0)
if cancel then return
cancel = cpar('writespec_xs',xtitle,20,'Xtitle',7,0)
if cancel then return
cancel = cpar('writespec_xs',ytitle,21,'Ytitle',7,0)
if cancel then return
cancel = cpar('writespec_xs',version,22,'Version',7,0)
if cancel then return
...
fxaddpar,hdr,'APRADIUS',strjoin(strcompress(apradius,/re),','), $
         ' Aperture radii in arcseconds'
if n_elements(BGR) ne 0 then fxaddpar,hdr,'BGR',bgr, $
  ' Background regions in arcseconds'
...
if n_elements(RMS) ne 0 then begin
    ...
    history =history+'  The RMS in angstroms of the wavelength calibration '+$
      'is '+strjoin(strcompress(rms*10^4,/RE),', ')+'.'
...
if keyword_set(FITS) then begin
    ...
    writefits,fullpath+'.fits',spectra,hdr
