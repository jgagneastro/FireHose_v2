...
;
; PURPOSE:
;     Extracts 2D extended source spectra from a XD spectral image.
;
; CATEGORY:
;     Spectroscopy
;
; CALLING SEQUENCE:
;     result = extractspec2d_xs(image,var,edgecoeffs,tracecoeffs,norders,$
;                               naps,start,stop,xranges,slith_arc,slith_pix,$
;                               apradii,SPATCOEFFS=spatcoeffs,BGORDER=bgorder,$
;                               BGR=bgr,BDPIXTHRESH=bdpixthresh,UPDATE=update,$
...
;     whose height is given by the maximum aperture size.
;
; EXAMPLE:
;
...
function extractspec2d_xs,image,var,edgecoeffs,tracecoeffs,norders,$
                          naps,start,stop,xranges,slith_arc,slith_pix,apradii,$
                          SPATCOEFFS=spatcoeffs,BGORDER=bgorder,BGR=bgr,$
...
if n_params() lt 12 then begin
    ...
    print, '                                apradii,SPATCOEFFS=spatcoeffs,$'
    print, '                                BGORDER=bgorder,BGR=bgr,$'
...
if cancel then return,-1
cancel = cpar('extract2dspec',apradii, 12,'Apradii',[2,3,4,5],[0,1])
if cancel then return,-1
...
mnspat   = round(2.0*max(apradii)/arcperpix)
mnspat   = (mnspat mod 2 eq 1) ? mnspat:(mnspat+1)
spectra = replicate(!values.f_nan,stop-start+1,2*mnspat,naps*norders)
...
;  Create the slit and psf mask
        ...
        mask = mkmask_xs(slity_arc,trace_arc,apradii,BG=bgr,CANCEL=cancel)
        if cancel then return,-1
            ...
            if debug and debugorder eq i and x[j] gt debugxrange[0] and x[j] $
              lt debugxrange[1] then begin
                ...
                for m = 0, naps-1 do begin
                    ...
                    plots,[trace_arc[m]-apradii[m],trace_arc[m]-apradii[m]],$
