function savitzky_golay,data,width,ERROR=error,DEGREE=degree,$
                        NLEFT=nl,NRIGHT=nr,DERIV_ORDER=order,$
                        COEFFICIENTS=filter_coef
;+
; NAME:
;       Savitzky_golay
;
...
;       Changed name to Savitzky_Golay M. Cushing 08/28/00
;       Added /CENTER, and /EDGE_TRUNCATE to convol function at end
;       M. Cushing 08/28/00
;-
        On_error,2
...
if n_elements(ERROR) ne 0 then $
  error = sqrt(convol(error^2,filter_coef^2,/CENTER,/EDGE_TRUNCATE ))

return, convol( data, filter_coef, /CENTER,/EDGE_TRUNCATE )
