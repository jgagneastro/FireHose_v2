...
;Computes the fit in the WV direction
xfit = fire_waveimg_ld(arcimg, arcivar, tset_slits, wstruct, savefile $
                    , fwhmset = fwhmset, qafile = qafile)
;stop;print,x_calcfit(1795, FITSTR=xfit);WAVELENGTH_TESTS
print,x_calcfit(1806, FITSTR=xfit),'C';WAVELENGTH_TESTS
;Computes the fit in the spatial direction
...
;Create a 2D wavelength solution with both informations
piximg = long_wpix2image(pixset, tset_slits, XFIT = xfit $
                         , waveimg = waveimg)

;plot,findgen(2048),poly(findgen(2048)/2048.,reform(*xfit.ffit))
;--------------                                
