...
function fire_localskysub_ld, sciimg, sciivar, skyimage, piximg, waveimg  $
                           ...
                           , NOLOCAL=nolocal,STD=STD, BOXCAR=boxcar
...
;                  AND obj_profiles[*, zz] LT 0.05*max(obj_profiles[*, zz])
   ...
   for ii = 0L, objwork-1L do begin
       ...
       single_struct = long_extract_optimal(thismask*waveimg $
          ...
          x_extobjbox, transpose(img_minsky), transpose(waveimg), $
                       var=transpose(1./sciivar), [objstruct.xpos[1024],1024],$
                       fin_spec, msk=msk, DEBUG=debug, wvmnx=[8000.,26000.],$
                       crval1=crval1, cdelt=cdelt1, npix=npix,$
                       TOT_TRC=objstruct.xpos, /REBINC, /CHK, aper=[10,10]
                       

          ; populate the same style of structure.  Bookkeeping only.
          single_struct = $
...
;      single_struct.trace += mincol
       ...
       if (keyword_set(struct_selecttags(objstruct[indx[iobj]], select_tags='WAVE_OPT')) LE 0) Then begin
          ...
          final_struct = objstruct[indx[iobj]]
          copy_struct, single_struct, final_struct
       endelse
