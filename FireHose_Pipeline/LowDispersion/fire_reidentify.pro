...
;JGAGNE HERE THIS MUST BE UNCOMMENTED !!!XXX!!!
lines0 = lines
restore, wstruct.REID_FILE
nl0 = n_elements(lines0)
nl = n_elements(lines)
for it=0L, nl-1L do begin
  void = min((lines0.wave-lines[it].wave)^2, wmin)
  if void gt 1d-2 then continue
  s0 = lines0[wmin]
  struct_assign, lines[it], s0
  lines0[wmin] = s0
endfor
lines = lines0
...
;   ;J.Gagne Aug. 2014; avoid crashing when lines are outside window
;   if (guesspix lt 0 or guesspix ge n_elements(xx)) then begin
...
;   endif
   ...
   ;J.Gagne Aug. 2014; avoid crashing when lines are outside window
   x_new = x_centspln(xx[(guesspix-10)>0L:(guesspix+10)<(n_elements(xx)-1L)],arc_obj_corr[(guesspix-10)>0L:(guesspix+10)<(n_elements(xx)-1L)], /silent)
