...
; 
; COMMON BLOCKS:
;       xatv_xystate:  contains variables describing the display state
;       xatv_xy_images: contains the internal copies of the display image
;       xatv_xy_color:  contains colormap vectors
;       xatv_xy_pdata:  contains plot and text annotation information
;
; RESTRICTIONS:
...
common xatv_xy_state, state, tot_state, nimg, flg_img, xout, yout, xfit, yfit, tset_slt, prof_fitstr_local, prof_fitbounds, ap_rad, ap_arcsec, ap_cen, ap_units, apcen_win, aprad_win, apunits_win, wintitle
common xatv_xy_color, r_vector, g_vector, b_vector
common xatv_xy_pdata, nplot, maxplot, plot_ptr
common xatv_xy_images, $
  main_image, $
  main_image1,   $
  main_image2,   $
  main_image3,   $
...
common xatv_xy_state
common xatv_xy_color
...
base = widget_base(title = wintitle, $
...
top_menu_desc = [ $
                  ...
                  {cw_pdmenu_s, 1, 'Apertures'}, $    ;Apertures menu
                  {cw_pdmenu_s, 2, 'Define center/width'}, $
                  {cw_pdmenu_s, 1, 'Help'}, $         ; help menu
                  {cw_pdmenu_s, 0, 'ATV Help'}, $
                  {cw_pdmenu_s, 2, 'Aperture Def Help'} $
                ]
...
widget_control, top_menu, event_pro = 'xatv_xy_topmenu_event'
widget_control, state.draw_widget_id, event_pro = 'xatv_xy_draw_color_event'
widget_control, state.draw_base_id, event_pro = 'xatv_xy_draw_base_event'
widget_control, state.keyboard_text_id, event_pro = 'xatv_xy_keyboard_event'
widget_control, state.pan_widget_id, event_pro = 'xatv_xy_pan_event'
...
; See the routine xatv_xy_stretchct to see why we do it this way.
...
pro xatv_xy_colorbar

; Routine to tv the colorbar at the bottom of the xatv window

common xatv_xy_state

xatv_xy_setwindow, state.colorbar_window_id
...
; xatv_xy_shutdown is called automatically by the xmanager, if xatv is
; killed by the window manager.

common xatv_xy_images
common xatv_xy_state
common xatv_xy_color
common xatv_xy_pdata

; Kill top-level base if it still exists
if (xregistered ('xatv_xy')) then widget_control, state.base_id, /destroy
...
delvarx, g_vector
delvarx, b_vector
delvarx, state, /free_mem
delvarx, tot_state, /free_mem
;delvarx, nimg
;delvarx, flg_img
;delvarx, xout
;delvarx, yout
...
pro xatv_xy_topmenu_event, event
...
; File menu options:
    'ReadFits': begin
        xatv_xy_readfits, newimage=newimage
        if (newimage EQ 1) then begin
            xatv_xy_getstats
            xatv_xy_settitle
            state.zoom_level =  0
            state.zoom_factor = 1.0
            if (state.default_autoscale EQ 1) then xatv_xy_autoscale
            xatv_xy_set_minmax
            xatv_xy_displayall
        endif
    end
    'WritePS' : xatv_xy_writeps
    'WriteTiff': xatv_xy_writetiff
    'Quit':     xatv_xy_shutdown
; ColorMap menu options:            
    'Grayscale': xatv_xy_getct, 0
    'Blue-White': xatv_xy_getct, 1
    'Red-Orange': xatv_xy_getct, 3
    'BGRY': xatv_xy_getct, 4
    'Rainbow': xatv_xy_getct, 13
    'Stern Special': xatv_xy_getct, 15
    'Green-White': xatv_xy_getct, 8
    'ATV Special': xatv_xy_makect, event_name
; Scaling options:
    'Linear': begin
        state.scaling = 0
        xatv_xy_displayall
...
; Label options:
    'TextLabel': xatv_xy_textlabel
    'Contour': xatv_xy_oplotcontour
    'Compass': xatv_xy_setcompass
    'ScaleBar': xatv_xy_setscalebar
    'EraseLast': xatverase, 1
    'EraseAll': xatverase
; Blink options:
    'SetBlink1': begin   
        xatv_xy_setwindow, state.draw_window_id
...
; Info options:
    'Photometry': xatv_xy_apphot
    'ImageHeader': xatv_xy_headinfo
    'Statistics': xatv_xy_showstats
...
; Coordinate system options:
    ...
    'RA,dec (J2000)': BEGIN 
       ...
       xatv_xy_gettrack             ; refresh coordinate window
    END 
    'RA,dec (B1950)': BEGIN 
       state.display_coord_sys = 'RA--'
...
; Help options:            
    'ATV Help': xatv_xy_help

    'Aperture Def Help': xatv_xy_aps_help

    'Define center/width': xatv_xy_setaps
...
if (xregistered('xatv_xy', /noshow)) then xatv_xy_resetwindow

end

;--------------------------------------------------------------------

pro xatv_xy_draw_color_event, event
...
case event.type of
    0: begin           ; button press
        if (event.press EQ 1) then begin
            state.cstretch = 1
            xatv_xy_stretchct, event.x, event.y, /getmouse
            xatv_xy_colorbar
        endif else begin
            xatv_xy_zoom, 'none', /recenter
        endelse
    end
    1: begin
        state.cstretch = 0  ; button release
        if (state.bitdepth EQ 24) then xatv_xy_refresh
        xatv_xy_draw_motion_event, event
    end
    2: begin                ; motion event
        if (state.cstretch EQ 1) then begin
            xatv_xy_stretchct, event.x, event.y, /getmouse 
            if (state.bitdepth EQ 24) then xatv_xy_refresh, /fast
        endif else begin 
            xatv_xy_draw_motion_event, event
        endelse
    end 
...
pro xatv_xy_draw_zoom_event, event
...
if (event.type EQ 0) then begin 
    case event.press of
        1: xatv_xy_zoom, 'in', /recenter
        2: xatv_xy_zoom, 'none', /recenter
        4: xatv_xy_zoom, 'out', /recenter
    endcase
endif

if (event.type EQ 2) then xatv_xy_draw_motion_event, event
...
pro xatv_xy_draw_blink_event, event
...
case event.type of
    ...
    2: xatv_xy_draw_motion_event, event ; motion event
endcase
...
pro xatv_xy_draw_phot_event, event
...
pro xatv_xy_draw_motion_event, event
...
pro xatv_xy_draw_base_event, event
...
pro xatv_xy_keyboard_event, event
...
case eventchar of
    '1': xatv_xy_move_cursor, eventchar
    '2': xatv_xy_move_cursor, eventchar
    '3': xatv_xy_move_cursor, eventchar
    '4': xatv_xy_move_cursor, eventchar
    '6': xatv_xy_move_cursor, eventchar
    '7': xatv_xy_move_cursor, eventchar
    '8': xatv_xy_move_cursor, eventchar
    '9': xatv_xy_move_cursor, eventchar
    'r': xatv_xy_rowplot
    'c': xatv_xy_colplot
    's': xatv_xy_surfplot
    't': xatv_xy_contourplot
    'p': xatv_xy_apphot
    'i': xatv_xy_showstats
    'q': xatv_xy_shutdown
;  Modified 6-23-01 JXP
    'z': xatv_xy_zoom, 'in', /recenter
    'Z': xatv_xy_zoom, 'out', /recenter
;  Wavelength options
    'S': xatv_xy_specplot
;  Loop through images
    'f': xatv_xy_nextimg, 0
...
;  Big pan
    ...
    '}': xatv_xy_bigpan, 'down'
    '[': xatv_xy_bigpan, 'left'
    ']': xatv_xy_bigpan, 'right'
;  Register
    'R': xatv_xy_register
    'L': xatv_xy_matchluts
;  Outputting XY coords
    ' ': xatv_xy_xyouts, event
    'P': xatv_xy_guessprof, event
    'C': xatv_xy_clearprof, event
    'O': xatv_xy_echprofset, event
    'H': xatv_xy_aps_help
    'A': xatv_xy_setaps
    'a': xatv_xy_setaps_ld
    'T': xatv_xy_profbox, event, 'T'
    'B': xatv_xy_profbox, event, 'B'
    else:  ;any other key press does nothing
endcase

if (xregistered('xatv_xy', /noshow)) then $
...
case event.type of
    ...
    2: begin
        xatv_xy_pantrack, event     ; motion event
        widget_control, state.pan_widget_id, /clear_events
    end
    else:
endcase
...
case uvalue of
    ...
    'mode': case event.index of
        0: widget_control, state.draw_widget_id, $
          event_pro = 'xatv_xy_draw_color_event'
        1: widget_control, state.draw_widget_id, $
          event_pro = 'xatv_xy_draw_zoom_event'
        2: widget_control, state.draw_widget_id, $
          event_pro = 'xatv_xy_draw_blink_event'
        3: widget_control, state.draw_widget_id, $
          event_pro = 'xatv_xy_draw_phot_event'
    ...
    'invert': begin                  ; invert the color table
        ...
        xatv_xy_stretchct, state.brightness, state.contrast
        if (state.bitdepth EQ 24) then xatv_xy_refresh
    end
    
    'restretch_button': xatv_xy_restretch

    'min_text': begin     ; text entry in 'min = ' box
        xatv_xy_get_minmax, uvalue, event.value
        xatv_xy_displayall
    end
    ...
    'zoom_in':  xatv_xy_zoom, 'in'         ; zoom buttons
    'zoom_out': xatv_xy_zoom, 'out'
    'zoom_one': xatv_xy_zoom, 'one'
...
;   Frame Number
    ...
    'frame_bg' : xatv_xy_setframe, event.value

    'done':  xatv_xy_shutdown
...
pro xatv_xy_message, msg_txt, msgtype=msgtype, window=window
...
; zoomed without a change in scaling, then just call xatv_xy_refresh
...
xatv_xy_setwindow, state.pan_pixmap
erase
tv, pan_image, state.pan_offset[0], state.pan_offset[1]
xatv_xy_resetwindow

xatv_xy_setwindow, state.pan_window_id
if (not(keyword_set(fast))) then erase
tv, pan_image, state.pan_offset[0], state.pan_offset[1]
xatv_xy_resetwindow
xatv_xy_drawbox, /norefresh

if (state.bitdepth EQ 24) then xatv_xy_colorbar

; redisplay the tracking image
if (not(keyword_set(fast))) then xatv_xy_gettrack
...
pro xatv_xy_move_cursor, direction
...
pro xatv_xy_get_minmax, uvalue, newvalue
...
pro xatv_xy_zoom, zchange, recenter = recenter
common xatv_xy_state
...
case zchange of
    ...
    else:  print,  'problem in xatv_xy_zoom!'
endcase
...
function xatv_xy_wcsstring, lon, lat, ctype, equinox, disp_type, disp_equinox, $
            disp_base60
...
xatv_xy_setwindow, state.track_window_id
tv, track_image
...
if (state.wcstype EQ 'angle') then begin
    ...
    wcsstring = xatv_xy_wcsstring(lon, lat, (*state.astr_ptr).ctype,  $
...
pro xatv_xy_drawbox, norefresh=norefresh
...
;----------------------------------------------------------------------
; routine to mark up aperture profile centers

pro xatv_xy_xyouts, event
...
pro xatv_xy_profbox, event, inchar
...
if (inchar EQ 'B') then begin
   if (n_elements(prof_fitbounds) LT 4) then begin
      prof_fitbounds = [xx,yy,-1,-1]
   endif else begin
      prof_fitbounds[0] = xx
      prof_fitbounds[1] = yy
   endelse
endif
if (inchar EQ 'T') then begin
   if (n_elements(prof_fitbounds) LT 4) then begin
      prof_fitbounds = [-1,-1,xx,yy]
   endif else begin
      prof_fitbounds[2] = xx
      prof_fitbounds[3] = yy
   endelse
...
if (prof_fitbounds[1] NE -1 AND prof_fitbounds[3] NE -1) then begin

   traceset2xy, tset_slt[0], rows, left_edge
   traceset2xy, tset_slt[1], rows, right_edge
   
   print, "Mapping wavelengths, please wait..."
   ximage = long_slits2x(tset_slt)
   omask = fire_ordermask(tset_slt)
   
; This is the order for each pixel that was flagged
   orderpix = omask[xx,yy]
   minwave = wave_image[prof_fitbounds[0],prof_fitbounds[1]]
   maxwave = wave_image[prof_fitbounds[2],prof_fitbounds[3]]
   
   order_ind=31L-orderpix
   width = median(right_edge[*, order_ind]-left_edge[*, order_ind])
   
   inprof = where(omask EQ orderpix AND wave_image GT minwave AND wave_image LT maxwave)
   
   xabs = ximage * width
   smash = fltarr(width)
   for ismh=0, n_elements(smash)-1 do begin
      thisx = where(round(xabs[inprof]) EQ ismh, nthis)
      if (nthis EQ 0) then continue
      smash[ismh] = total(main_image1[inprof[thisx]])/(nthis > 1.0)
   endfor
   
; zeropoint correction and outlier rejection in smash
   smashsig = robust_sigma(smash)
   junk = djs_reject(smash,median(smash)*(fltarr(n_elements(smash))+1),outmask=mask, sigma=smashsig, lower=3., upper=5.)
   smash= (smash-median(smash[mask]))/abs(total(smash[mask]))


   fit = x1dfit(findgen(width)/width, smash, func='BSPLIN', /inter, FITSTR=fitstr)
   prof_fitstr_local = fitstr
...
;----------------------------------------------------------------------
; This is primarily for low dispersion mode, fitting
; a polynomial to the xout/yout pixel combinations

pro xatv_xy_guessprof, event
...
; This is the order for each pixel that was flagged
orderpix = omask[xout, yout]
;gd = orderpix[where(orderpix GE 11 AND orderpix LE 31, ngd)]
gd = where(orderpix GE 11 AND orderpix LE 31, ngd)

if (ngd EQ 0) then begin
   return
endif

if (n_elements(xout) GE 1) then begin
   xfrac = (xout[gd]-left_edge[yout[gd],31-orderpix[gd]]) / $
           (right_edge[yout[gd],31-orderpix[gd]]-$
            left_edge[yout[gd],31-orderpix[gd]])

   ; Fit xfrac with the order number 
   ; to compensate for atmospheric dispersion
   if (n_elements(xfrac) EQ 1) then begin
      xprof = xfrac 
   endif else begin
      ; HACK
      if (n_elements(xfrac) LE 2) then begin
         xprof = total(xfrac)/2.0
      endif 
      if (n_elements(xfrac) EQ 3) then begin
         mxb = linfit(orderpix, xfrac)
         xprof = poly(orderpix,mxb)
      endif 
      if (n_elements(xfrac) GT 3) then begin
         tt = x1dfit(orderpix, xfrac, /inter, fitstr=fitstr)
         xprof = x_calcfit(orderpix,fitstr=fitstr)
      endif 
   endelse
...
;   stop
   ...
   for i=0, 2047 do begin
      for ord = 0,20 do begin
         ll = left_edge[i,ord] > 0
         rr = right_edge[i,ord] > 1
         xpix = where(omask[ll:rr,i] EQ 31L-ord AND $
                      abs(ximage[ll:rr,i]-xprof[0]) EQ $
                      min(abs(ximage[ll:rr,i]-xprof[0])), nx)
         if (nx EQ 1 OR nx EQ 2) then begin
            if (nx EQ 1) then begin
               xfit[i,ord] = ll+xpix
            endif else begin ; occasionally exactly equal
               xfit[i,ord] = ll+xpix[0]
            endelse
         endif else begin
            if (ll EQ 0 OR rr LT 1) then begin
               continue
            endif else begin
;               stop
            endelse
         endelse
         main_image[xfit[i,ord],i] = -10000
      endfor
   endfor
...
pro xatv_xy_setaps_event, ev
  ...
  case uvalue of
     ...
     'apunit_dropbox' : begin
        ap_units = ev.index

        if (ap_units EQ 0) then begin
           if (NOT keyword_set(ap_arcsec)) then ap_arcsec = 1.0
           widget_control, aprad_win, set_value=strtrim(ap_arcsec,2)
        endif else begin
           if (n_elements(ap_rad) EQ 1) then begin
              widget_control, aprad_win, set_value=strtrim(ap_rad,2)
           endif else begin
              widget_control, aprad_win, set_value=strtrim(ap_rad[0],2)
           endelse
        endelse
        return
     end 
     ...
     'quit_aps' : begin
        ...
        widget_control, apcen_win, get_value=apcen_tmp
        widget_control, aprad_win, get_value=aprad_tmp

        if (apcen_tmp LT 0.05) then begin
           val = dialog_message(["Aperture center is too close to slit edge.",$
                                 "Setting to 5%"], /cancel)
        ...
        if (apcen_tmp GT 0.95) then begin
           val = dialog_message(["Aperture center is too close to slit edge.",$
                                 "Setting to 95%"], /cancel)
           if (val EQ 'Cancel') then begin
              return
           endif else begin
              ap_cen = 0.95
        ...
        if (ap_units EQ 1) then begin
           if (aprad_tmp GT 30) then begin
              val = dialog_message(["Aperture radius is too large.",$
                                    "Setting to 30 pixels"], /cancel)
              if (val EQ 'Cancel') then begin
                 return
              endif else begin
                 ap_rad = 30
           ...
           if (aprad_tmp LT 0) then begin
              val = dialog_message(["Aperture radius must be positive integer.",$
                                    "Setting to 1 pixel"], /cancel)
              if (val EQ 'Cancel') then begin
                 return
              endif else begin
                 ap_rad = 1
           ...
           if (n_elements(ap_rad) EQ 1) then begin
              ap_rad = replicate(ap_rad, 21)
           endif else begin
              ap_rad = replicate(ap_rad[0], 21)
           endelse
        endif

        if (ap_units EQ 0) then begin

           ; this is measured straight from a flat field
;           slitwidth_pixels = [78,77,76,74,73,71,71,69,66,65,64,63,63,61,61,60,59,58,54,54,52]

           ; a least squares polynomial fit to slitwidth_pixels
           slitwidth_fit = [78.1745,76.7884,75.4161,74.0577,72.7131,71.3824,70.0656,68.7625,67.4734,66.1981,64.9366,63.6890,62.4552,61.2353,60.0292,58.8370,57.6586,56.4941,55.3434,54.2066,53.0836]
           arcsec_per_pixel = 6.0 / slitwidth_fit

           ap_rad = aprad_tmp[0] / 2.35 * 2 / arcsec_per_pixel
  ...
  ; Update the GUI
        ...
        for i=0, 2047 do begin
           for ord = 0,20 do begin
              ...
              rr = right_edge[i,ord] > 1
              xpix = where(omask[ll:rr,i] EQ 31L-ord AND $
                           abs(ximage[ll:rr,i]-ap_cen[0]) EQ $
                           min(abs(ximage[ll:rr,i]-ap_cen[0])), nx)
              if (nx EQ 1 OR nx EQ 2) then begin
                 if (nx EQ 1) then begin
                    xfit[i,ord] = ll+xpix
                 endif else begin ; occasionally exactly equal
...
;                    stop
              ...
              if ((i MOD 2) NE 0) then begin
                 main_image[(xfit[i,ord]+ap_rad[ord] < 2047),i] = -10000
                 main_image[(xfit[i,ord]-ap_rad[ord] > 0),i] = -10000
              endif
           endfor
        endfor
        xatv_xy_scaleimage
     ...
     'edit_apcen': begin
        widget_control, apcen_win, get_value=tmp
        ap_cen = tmp
        return
     end
     ...
     'edit_aprad': begin
        if (ap_units EQ 1) then begin
           widget_control, aprad_win, get_value=tmp
           ap_rad = tmp
        endif else begin
           widget_control, aprad_win, get_value=tmp
           ap_arcsec = tmp
...
pro xatv_xy_setaps
  ...
  apsbase = WIDGET_BASE(title="Aperture Settings", /column)
  
  ab1 = widget_base(apsbase,/row)
  wt1 = widget_label(ab1, Value="Aperture Center (slit frac):")

  if keyword_set(ap_cen) then begin
     tmp = strtrim(ap_cen,2)
  endif else begin
     tmp = strtrim(string(0.5),2)
     ap_cen = 0.5
  endelse
  apcen_win = widget_text(ab1, Value=tmp, xsize=7, /editable, /all_events, uvalue='edit_apcen')

  ab2 = widget_base(apsbase,/row)
  if keyword_set(ap_rad) then begin
     if (ap_units EQ 0) then begin
        if (keyword_set(ap_arcsec) EQ 0) then ap_arcsec = 1.0
        tmp = strtrim(ap_arcsec,2)
     endif else begin
        if (n_elements(ap_rad) EQ 1) then begin
           tmp = strtrim(ap_rad,2)
        endif else begin
           tmp = strtrim(ap_rad[0],2)
        endelse
     endelse
  endif else begin 
     if (ap_units EQ 0) then begin
        tmp = strtrim(string(1),2)
     endif else begin
        tmp = strtrim(string(10),2)
     endelse
     ap_rad = 10
     ap_arcsec = 1.0
  endelse
  wt2 = widget_label(ab2, Value="Aperture Size              :")
  aprad_win = widget_text(ab2, Value=string(tmp) ,xsize=7, /editable, /all_events, uval='edit_aprad')

  ab3 = widget_base(apsbase,/row)
  wt3 = widget_label(ab3, Value="Measured in                :")
  apunits_win = WIDGET_COMBOBOX(ab3,uname='aps_units', Value=["Arcsec (FWHM)","Pixels (Radius)"],uval='apunit_dropbox')
  widget_control, apunits_win,set_combobox_select=ap_units

  ab3 = widget_base(apsbase, /row)
  apsquit = WIDGET_BUTTON(ab3, Value='Accept', uvalue='quit_aps')
  apscancel = WIDGET_BUTTON(ab3, Value='Cancel', uvalue='cancel_aps')

  WIDGET_CONTROL, apsbase, /realize
  xmanager, 'xatv_xy_setaps', apsbase

end

;---------------------------------------------------------------------

pro xatv_xy_setaps_ld_event, ev
  ...
  ; Update the GUI
        ydot = findgen(1024)*2
        main_image = main_image1
        main_image[xfit,indgen(2048)] = -10000
        if (n_elements(xfit) GT 0) then begin
           main_image[xfit[ydot]-ap_rad[0],ydot] = -10000
           main_image[xfit[ydot]+ap_rad[0],ydot] = -10000
        endif else begin
           if (n_elements(xout) GT 0) then begin
              if (ap_units EQ 1) then begin
                 main_image[xout-ap_rad[0],ydot] = -10000
                 main_image[xout+ap_rad[0],ydot] = -10000
              endif
           endif
        endelse
...
pro xatv_xy_setaps_ld

  common xatv_xy_state

  apsbase = WIDGET_BASE(title="Aperture Settings (Prism Mode)", /column)
  ...
  WIDGET_CONTROL, apsbase, /realize
  xmanager, 'xatv_xy_setaps_ld', apsbase
...
pro xatv_xy_getstats, align=align
...
pro xatv_xy_setwindow, windowid
...
; (i.e. non-xatv) idl window.  Use xatv_xy_setwindow to set the window to
; one of the xatv window, then display something to that window, then
; use xatv_xy_resetwindow to set the current window back to the currently
...
pro xatv_xy_readfits, fitsfilename=fitsfilename, newimage=newimage, $
                   EXTEN=exten
...
if (n_elements(strcompress(head, /remove_all)) LT 2) then begin
    xatv_xy_message, 'File does not appear to be a valid fits image!', $
...
if (numext EQ 0 AND naxis LT 2) then begin
    xatv_xy_message, 'Selected file is not a 2-d FITS image!', $
      window = window, msgtype = 'error'
    return
endif
...
if ((numext GT 0) AND (instrume NE 'WFPC2')) then begin
    xatv_xy_fitsext_read, fitsfile, numext, head, cancelled
endif else if ((instrume EQ 'WFPC2') AND (naxis EQ 3)) then begin
    xatv_xy_wfpc2_read, fitsfile, head, cancelled
endif else if ((naxis EQ 3) AND (origin EQ '2MASS')) then begin
    xatv_xy_2mass_read, fitsfile, head, cancelled
endif else begin
    xatv_xy_plainfits_read, fitsfile, head, cancelled, EXTEN=exten
endelse
...
pro xatv_xy_fitsext_read, fitsfile, numext, head, cancelled
...
if (xten EQ 'BINTABLE') then begin
    xatv_xy_message, 'File appears to be a FITS table, not an image.', $
...
pro xatv_xy_plainfits_read, fitsfile, head, cancelled, EXTEN=exten
...
pro xatv_xy_wfpc2_read, fitsfile, head, cancelled
...
pro xatv_xy_2mass_read, fitsfile, head, cancelled
...
if (filename EQ state.current_dir) then begin
    xatv_xy_message, 'Must indicate filename to save.', msgtype = 'error', /window
    return
endif
...
pro xatv_xy_stretchct, brightness, contrast,  getmouse = getmouse
...
function xatv_xy_polycolor, p

; Routine to return an vector of length !d.table_size-8,
; defined by a 5th order polynomial.   Called by xatv_xy_makect
...
;   widget_control, state.base_id, tlb_set_title = 'xatv_xy'
   widget_control, state.base_id, tlb_set_title = wintitle
...
if (xregistered('xatv_xy_headinfo')) then begin
    widget_control, state.headinfo_base_id, /destroy
endif

if (xregistered('xatv_xy_stats')) then begin
...
if (not(ptr_valid(state.head_ptr))) then begin
    ...
    xatv_xy_message, 'No header information available for this image!', $
      msgtype = 'error', /window
    return
endif
...
if (not(xregistered('xatv_xy_headinfo', /noshow))) then begin
    ...
    xmanager, 'xatv_xy_headinfo', headinfo_base, /no_block
...
pro xatv_xy_headinfo_event, event
...
pro xatv_xy_plot1contour, iplot
common xatv_xy_pdata
common xatv_xy_state
...
pro xatv_xy_plot1compass, iplot
...
pro xatv_xy_plot1scalebar, iplot
...
pro xatv_xy_arcbar, hdr, arclen, LABEL = label, SIZE = size, THICK = thick, $
...
for iplot = 1, nplot do begin
    case (*(plot_ptr[iplot])).type of
        'points'  : xatv_xy_plot1plot, iplot
        'text'    : xatv_xy_plot1text, iplot
        'contour' : xatv_xy_plot1contour, iplot
        'compass' : xatv_xy_plot1compass, iplot
        'scalebar': xatv_xy_plot1scalebar, iplot
        else      : print, 'Problem in xatv_xy_plotall!'   
    endcase
endfor
...
if (not(xregistered('xatv_xy', /noshow))) then begin
...
;  convert color names to index numbers, and set default=red
   ...
   options.color = xatv_xy_icolor(options.color)
...
;  convert color names to index numbers, and set default=red
   ...
   options.c_color = xatv_xy_icolor(options.c_color)
...
if (NOT keyword_set(norefresh)) then xatv_xy_refresh

end

;----------------------------------------------------------------------

pro xatv_xy_textlabel
...
if (nplot GE maxplot) then begin
    xatv_xy_message, 'Total allowed number of overplots exceeded.', $
      msgtype = 'error', /window
    return
endif
...
if (state.wcstype NE 'angle') then begin 
    xatv_xy_message, 'Cannot get coordinate info for this image!', $
      msgtype = 'error', /window
    return
endif
...
xatv_xy_plotwindow
xatv_xy_plot1compass, nplot

end

;---------------------------------------------------------------------

pro xatv_xy_setscalebar
...
xmanager, 'xatv_xy_lineplot', state.lineplot_base_id, /no_block
...
;if (not (xregistered('xatv_xy_lineplot', /noshow))) then begin
;    xatv_xy_lineplot_init
;endif

;xatv_xy_setwindow, state.lineplot_window_id
;erase
...
pro xatv_xy_lineplot_event, event
...
pro xatv_xy_aps_help

  helptxt = [ $
            "", $
            "FIREHOSE Aperture Selection GUI", $
            "----------------------------------------", $
            "", $
            "SPACE: Manually specify an object trace point", $
            "C    : Clear all object trace points", $
            "B    : Define bottom of a wv region for obj prof fit", $
            "T    : Define top of a wv region for obj prof fit", $
            " ", $
            "High Dispersion Mode Only", $
            "--------------------------", $
            "O    : Use trace points and order boundaries to define full trace", $
            " ", $
            " ", $
            "Low-Dispersion Mode Only:", $
            " -----------------------", $
            "P    : Fit polynomial to object trace points (FIRE Prism-mode only)", $
            " ", $
            " ", $
            " ", $
            " ", $
...
i = i + 1
h[i] = 'xatv_xy_shutdown:   quits xatv'
i = i + 1
h[i] = 'NOTE: If xatv should crash, type xatv_xy_shutdown at the idl prompt.'
i = i + 5
...
if (not (xregistered('xatv_xy_help', /noshow))) then begin
...
helptitle = strcompress('xatv v' + state.version + ' help')
    ...
    xmanager, 'xatv_xy_help', help_base, /no_block
...
if (not (xregistered('xatv_xy_stats', /noshow))) then begin
    ...
    xmanager, 'xatv_xy_stats', stats_base, /no_block
...
xatv_xy_setwindow, state.statzoom_window_id
erase
...
pro xatv_xy_imcenterf, xcen, ycen
...
function xatv_xy_splinefwhm, rad, prof, splrad, splprof
...
fwhm = xatv_xy_splinefwhm(out[*,0],out[*,6])
...
; Run xatv_xy_radplotf and plot the results

xatv_xy_setwindow, state.radplot_window_id
xatv_xy_radplotf, x, y, fwhm
...
;      clon, clat
;    wcsstring = xatv_xy_wcsstring(clon, clat, (*state.astr_ptr).ctype,  $
...
xatv_xy_setwindow, state.photzoom_window_id
erase
...
pro xatv_xy_apphot_event, event
...
case uvalue of
    ...
    'photsettings': xatv_xy_apphot_settings
...
if (not (xregistered('xatv_xy_apphot', /noshow))) then begin
    ...
    xmanager, 'xatv_xy_apphot', apphot_base, /no_block
...
;--------------------------------------------------------------------
pro xatv_xy_setframe, id, NOREAD=noread
...
;pro xatv_xy_newimg, img, ndisplay

;common xatv_xy_state
;common xatv_xy_images
;
;if ( (n_params() LT 2) AND (xregistered('xatv_xy', /noshow))) then begin
;    print, 'USAGE: xatv_xy_newimg, array_name OR fitsfile, frameno'
;    return
;endif
;  
;
...
; Routine to pan in a large way on the image
  ...
  case direct of
      ...
      else:  print,  'problem in xatv_xy_bigpan!'
  endcase
...
; then run xatv_xy_startup to create the widgets.  If ATV already exists,
...
if (keyword_set(TSET_SLITS)) then tset_slt=tset_slits
...
if ( (n_params() EQ 0) AND (xregistered('xatv_xy', /noshow))) then begin
...
if keyword_set( frameno ) then begin
    ...
    nimg = frameno
    flg_img[nimg] = 1
    xatv_xy_setframe, nimg, /noread
    widget_control, state.frame_id, set_value=nimg
endif

; Before starting up xatv, get the user's external window id.  We can't
; use the xatv_xy_getwindow routine yet because we haven't run xatv
; startup.  A subtle issue: xatv_xy_resetwindow won't work the first time
...
; this routine.
if (not (xregistered('xatv_xy', /noshow)) OR keyword_set(FORCESTART)) then begin
   userwindow = !d.window
   xatv_xy_startup
...
if ( (n_params() NE 0) AND (size(image, /tname) EQ 'STRING')) then begin
    ...
    if (count EQ 0) then begin
        ...
        xatv_xy_readfits, fitsfilename=ifexists[0], newimage=newimage, $
          EXTEN=exten
    endelse
endif
...
ap_units = 0 ;  0 = arcsec FWHM, 1 = pixels

; Register the widget with xmanager if it's not already registered
if (not(xregistered('xatv_xy', /noshow))) then begin
    nb = abs(block - 1)
    xmanager, 'xatv_xy', state.base_id, no_block = nb, cleanup = 'xatv_xy_shutdown'
    wset, userwindow
...
if ((size(xfit))[1] EQ 2048) then begin
   retx = xfit
   rety = findgen(2048)
endif else begin
   if (n_elements(xout) GT 0) then begin
...
if (n_elements(prof_fitstr_local) GT 0) then begin
   prof_fitstr = prof_fitstr_local
endif

if (n_elements(ap_rad) EQ 0) then begin
   box_rad = 10L                ; Default
endif else begin
   box_rad = ap_rad
endelse
