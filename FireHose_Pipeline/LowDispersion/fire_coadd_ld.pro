...
PRO FIRE_COADD_ld, infiles, objid, OUTFIL = OUTFIL $
                , IREF = IREF $
                ...
                , BOX = BOX, NOSHIFT = NOSHIFT, NOREJ = NOREJ $
...
IF n_elements(objid) EQ 1 THEN objid = replicate(objid-1L, nimgs) $
ELSE IF n_elements(objid) EQ nimgs THEN objid = objid-1L $
...
;    dum = long_extinct(obj[objid[j]].wave_opt, head0, exptime = exptime1)
;    exptime[j] = exptime1
    exptime[j] = 0
...
long_combspec, influx, inivar, inloglam, insky = insky, innivar = innivar $
               ...
               , NOSHIFT = NOSHIFT, NOREJ = NOREJ $
               ,MEDSCALE=MEDSCALE $
               , SN2 = SN2, YMULT = YMULT, DEBUG = DEBUG

;; I'm commenting out this last step, since I realize it undoes the 
...
IF keyword_set(OUTFIL) THEN BEGIN
    ...
    sxaddpar, head0, 'EXPTIME_AVG', total(exptime)/double(nimgs)
...
IF KEYWORD_SET(SKYFIL) THEN BEGIN
    sxaddpar, head0, 'NEXP', nimgs
    sxaddpar, head0, 'EXPT_AVG', total(exptime)/double(nimgs)
    sxaddpar, head0, 'BITPIX', -32
    sxaddpar, head0, 'NAXIS', 1
    sxaddpar, head0, 'NAXIS1', n_elements(newflux)
    sxdelpar, head0, 'NAXIS2'
