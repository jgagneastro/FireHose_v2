...
pro f_extechopt, img, skysub, ivar, ordr_str, obj_str, velpix, $
...
;  fi    = 1.0/(fvar + (fvar EQ 0)) * (fvar GT 0)
  fi = 1.0/extract_asymbox2(var, left_edge, right_edge)
...
;     aper    = obj_str[q].aper > base_aper
      ...
      ywave = x_qckwav(slit_pos, ystart, ordr_shift[q].arc_m, $
                       arc_slope=arc_slope)
      ypix = x_qckwav(slit_pos, ystart, ordr_shift[q].arc_m, $
                       arc_slope=arc_slope)


      ywave = img_arc[xstart,ystart]
      ;;
      ...
      ; Here's the problem!!!!!!!!!!!  What is the problem??  JJB
      ...
      if keyword_set(img_arc) then begin
          ...
          zero_wave = where(wave_order LT 3.0 OR (abs(wave_diff) GT 0.03))
          ...
          raw_wave_set = bspline_iterfit(ypix, 1.0d*wave_order, $
                                         invvar=1.0d*(wave_order GT 3), $
                                         nbkpt=10, /silent, /double, yfit=ywv)
...
;          fvar  = fvar*(fmask EQ 0) - (fmask GT 0)
          ...
          ybox = x_qckwav( objcen - slit_cen, ycol, ordr_shift[q].arc_m)
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
          ...
          fx_model = bspline_valu(ypix, fx_set) 
          fc_model = bspline_valu(ypix, fc_set) 
     ...
     if nin GT 3*smooth_length then begin
      ...
      if trace_sn GT 10.0 then begin
        ...
        if trace_sn GT 50 then begin
          ...
          trace_corr = bspline_valu(bspline_valu(ycol,dbl_wave_set), trace_dev_set)
        endif else begin
        ...
        if keyword_set(chk) then begin 
          ...
          plot, 10^ywave[in[ks]], trace_deviation,ps=3, yr=[-0.5, 0.5], /xs, $
             ytitle='Pixel Deviation', xtitle='Pixel Row', $
             title='Residuals to profile center Fit for Order'+ $
                          string(ordr_str[q].order, format='(i4)')

          oplot, 10^bspline_valu(ycol,dbl_wave_set), trace_corr, color=clr.red
          oplot, 10^bspline_valu(ycol,dbl_wave_set), damped_corr, color=clr.green
...
;  set_plot, 'ps'
;  device, filename=STRTRIM(STRING(ordr_str[q].order),2)+'crap.ps', /color
           plot, slit_dist, obj_profile, /nodata, color=clr.white, $
              ...
              ytitle='Object Profile Cross-section', charsize=1.2
...
;;
     npix = long((maxlog - minlog)/loglam - 1) > 2 <2048
