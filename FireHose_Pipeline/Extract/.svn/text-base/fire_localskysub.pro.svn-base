...
function fire_localskysub, sciimg, sciivar, skyimage, piximg, waveimg  $
...
;;-----------------------------------------------
    ...
    while i1 LT nobj do begin
      group = i1
      ; RAS: there was a bug here...should use order center
      ; to define min and max, not the object position!
      ; This may have to do with FIRE's short slit.
      if (nobj GT 1) then begin
         print, "FIRE_LOCALSKYSUB: multiple object mode has been disabled.  Consult an expert"
         stop
...
;            if (i1 EQ 19) then stop
            ...
            if (keyword_set(PROFILE_INPUT)) then begin
               obj_profiles[*] = profile_input
               xnew = objstruct[indx[iobj]].xpos - mincol
               fwhmfit = fltarr(n_elements(xnew)) + objstruct[indx[iobj]].fwhm
               med_sn2 = median(flux*flux*fluxivar)
            endif else begin
               obj_profiles[ipix, ii] = $
                  fire_gprofile(img_minsky[ipix] $
                                , (modelivar*mask)[ipix] $
                                , waveimg[ipix] $         
                                , objstruct[indx[iobj]].xpos - mincol $
                                , wave, flux, fluxivar $
                                , objstruct[indx[iobj]] $
                                , hwidth = objstruct[indx[iobj]].maskwidth $
                                , fwhmfit = fwhmfit $
                                , thisfwhm = objstruct[indx[iobj]].fwhm $
                                , xnew = xnew, nccd = nccd $
                                , SN_GAUSS = SN_GAUSS $
...
;       Use spatial profile of the sky to determine the SKY_MODEL
        sky_bmodel = 0.0
        iterbsp = 0
        WHILE total(sky_bmodel) EQ 0.0 AND iterbsp LE 5 and $
           not keyword_set(NOLOCAL) DO BEGIN
...
;                  AND obj_profiles[*, zz] LT 0.05*max(obj_profiles[*, zz])
            ...
            inmask = bytarr(2048,2048)+1B

            tt = lindgen(2048)
            ; RAS BUG FIND??
;            for delt=(-1*objstruct[ii].aper[0]),objstruct[ii].aper[1] do begin
;               inmask[objstruct[ii].xpos[tt]+delt,tt] = 0B
;            endfor
;           REPLACEMENT CODE FOLLOWS           
            for delt=(-1*objstruct[indx[iobj]].aper[0]),objstruct[indx[iobj]].aper[1] do begin
               inmask[objstruct[indx[iobj]].xpos[tt]+delt,tt] = 0B
            endfor
...
;        stop
;        plt = where(outmask NE 0)
;        x_splot, piximg[plt], sciimg[plt], psym1=3, ytwo=skyimage[plt], psym2=3, /block

        if (0) then begin
           xatv, thismask*(sciimg-skyimage), /block, min=-100, max=100
        endif
...
;      single_struct.trace += mincol
   ...
   IF KEYWORD_SET(CHK) THEN begin
      xatv, thismask*(sciimg-skyimage), /block $
