...
FUNCTION fire_get, data, ONEARC=onearc, STD=std, SCIIMG=sciimg, $
	...
	if skip_get EQ 0 then begin
		if obj_or_not EQ 1 then begin ;; data is an object reference
			...
			;J. Gagne !@!@!@!
			;if n_tags(keys) eq 1 then $
			  ;if (tag_names(keys))[0] eq 'ARCFITS' or (tag_names(keys))[0] eq 'ARCIMGFITS' then begin; or (tag_names(keys))[0] eq 'TARCFITS' or (tag_names(keys))[0] eq 'TARCIMGFITS'
			    ;if strtrim(val,2) ne '' then $
			      ;if (strsplit(val,path_sep(),/extract))[0] ne 'redux' then begin
			        ;stop
			        ;val = 'redux'+path_sep()+val
            ;endif
			  ;endif
		endelse
	endif else begin
