...
common x_splot_common, xdat, ydat, y2, x2, y3, x3, y4, x4
  ...
  case uval of
      ...
      'TEXT' : begin
          ...
          case eventch of
              ...
              'd': begin
                 if (state.flg_y3 EQ 1) then begin
                    xkill = xgetx_plt(state, /str)
                    dif = abs(x3-xkill)
                    mtch = where(dif EQ min(dif), nmtch)
                    if (nmtch EQ 1) then begin
                       x3 = x3[where(dif NE min(dif))]
                       y3 = y3[where(dif NE min(dif))]
                    endif
                 endif
              end
              ...
              'a': begin
                 if (state.flg_y3 EQ 1) then begin
                    xguess = xgetx_plt(state, /str)
                    yguess = xgety_plt(state, /str)
;                    ind = where(abs(xdat-xguess) EQ min(abs(xdat-xguess)))
;                    cen = f_centspln(xdat[ind[0]-10:ind[0]+10], ydat[ind[0]-10:ind[0]+10], /SILENT, /FORCE)-2.0 ; the minus 2 is for buffer.
;                    x3tmp = [x3,cen]
                    x3tmp = [x3,xguess]
;                    ind = where(abs(xdat-cen) EQ min(abs(xdat-cen)))
;                    y3tmp = [y3, ydat[ind[0]]]
                    ind = where(abs(xdat-xguess) EQ min(abs(xdat-xguess)))
                    y3tmp = [y3, ydat[ind[0]]]
                    x3 = x3tmp[sort(x3tmp)]
...
; y4
  ...
  if state.flg_y4 then begin
      oplot, x4, y4, color=color.cyan, psym=state.psym4
  endif
...
pro f_splot, xin, yin, XSIZE=xsize, YSIZE=ysize, TITLE=title, YTWO=ytwo, $
             PSYM2=psym2, BLOCK=block, XTWO=xtwo, PSYM1=psym1, XYOFF=xyoff, $
             XMNX = XMNX, YMNX = ymnx, XTHR = xthr, YTHR = ythr, $
             PSYM3 = psym3, XFOU = XFOU, YFOU = YFOU, PSYM4 = PSYM4
...
; Send to the xmanager
  ...
  if keyword_set( X3 ) then xthr = x3
  if keyword_set( Y3 ) then ythr = y3
