function fire_superfast_superflat, filenames, slitfile, pixfile $
  ...
  prog_name = 'fire_superfast_superflat';
  ...
  IF NOT KEYWORD_SET(order_vec) THEN BEGIN 
     order_vec = reverse(dindgen(21) + 11)
     norders = 21L
  ...
  FIRE_COMBINE_FLATS, filenames, ordermask, edgmask, IMAGE=image, OUTMASK=outmask, gain=gain
  ...
  IF NOT KEYWORD_SET(NOSCAT) THEN begin
     ...
     print, " "
;     stop
     scatt_model = fire_scattlight(image, tset_slits, scatt_shift=scatt_shift)
  endif
...
;      stop
      ...
      IF (KEYWORD_SET(TWEAK)) THEN BEGIN
         ...
         IF xleft_tweak NE -1.0 THEN BEGIN
            ...
            splog, 'Tweaking position of LEFT boundary for order', 32L-iorder
         ENDIF
         ...
         IF xright_tweak NE -1.0 THEN BEGIN
           ...
           splog, 'Tweaking position of RIGHT boundary for order', 32L-iorder
         ENDIF  
      ENDIF
      IF KEYWORD_SET(CHK) THEN BEGIN
         ...
         rrind = (sort(randomn(seed,n_elements(psort2))))[0:1000]
         plot, ximg[fitpix2[psort2[rrind]]], nrm_flat_orig[rrind] $
