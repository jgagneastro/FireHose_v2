...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
function f_fit2darc, arcfil, ordr_str, arc_info, nycoeff=nycoeff, $
                     nocoeff=nocoeff, CLOBBER=clobber, SZ=sz, $
                     DEBUG=debug, CHKRES=chkres, out_str=out_str, $
                     QAFIL=qafil, YVAL=yval, ORIG=orig, SIGREJ=sigrej $
                     , pixrms = pixrms
...
; SETUP THE DATA
   ...
   gd_ordr = where(sv_lines.nlin NE 0 AND $
                   ordr_str.flg_anly NE (-1) AND $
                   ptr_valid(all_arcfit.ffit), ngd_ordr)
...
;           dwv = abs(10^wv[0]-10^wv[npix-1])/float(all_pix[npix-1])
	...
	;; To avoid a pointer leak, free the pointers from all_arcfit
