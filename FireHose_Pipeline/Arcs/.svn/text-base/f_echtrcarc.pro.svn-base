...
;-
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
function f_echtrcarc, arc_fil, ordr_str, out_fil, SZCCD=szccd, $
...
;
  if  N_params() LT 3  then begin 
      print,'Syntax - ' + $
        'rslt = m_echtrcarc(arc_fil, ordr_str, out_fil, ' + $
...
;  Optional Keywords
  ...
  if not keyword_set( SAT ) then sat = 30000.
  if not keyword_set( SZCCD ) then szccd = [2048L,2048L]
;  if not keyword_set( XOFF ) then xoff =  0.
  ...
  ;; Arc file
  print, 'f_echtrcarc: Reading arc: ', arc_fil
...
;  slopecoeff = fltarr(nxcoeff, norders)
  ...
  tmp = { $
          nslit: 0L, $
          ntrc: 0L, $
          msk: bytarr(800L,800L), $  ;these used to be 300
          x1: fltarr(800L,800L), $
          xfit: fltarr(800L, 800L) $
        }
...
;;
  ...
  ;; Output
  print, 'f_echtrcarc: Writing '+out_fil
