...
for i = 0, state.r.nfilters-1 do begin
   ...
   if result eq 0 or result eq 1 then begin
        ...
        mc_syntphot,spec[*,0]*10000.,spec[*,1],*state.d.wvega*10000.,*state.d.fvega,$
                    state.r.vegazero[state.r.currentsystem],trans[*,0]*10000., $
                    trans[*,1],bandwidth,l_eff,l_ave,flux,mag,$
...
;  Clear table.
  ...
  widget_control, state.w.coltable,SET_VALUE=strarr(6), $
...
;  Read spectra and load data.
  ...
  *state.d.hdr  = hdr
  ...
  mc_getwunits,state.r.iwunits,wunit,pwunit
  pfunit = mc_getfunits(state.r.ifunits)

  state.p.xunits = pwunit
  state.p.yunits = pfunit
...
;  Load the three structures in the state structure.
  ...
  state.w.xmc_syntphot_base = widget_base(TITLE='Xmc_Syntphot', $
     ...
     button = widget_button(state.w.xmc_syntphot_base,$
                            FONT=buttonfont,$
        ...
        col1_base = widget_base(row1_base,$
           ...
           row = widget_base(col1_base,$
              ...
              fld = coyote_field2(row,$
                                  ...
                                  XSIZE=18,$
                                  TEXTID=textid)
              state.w.spectrum_fld = [fld,textid]

              state.w.iwave_dl = widget_droplist(col1_base,$
                                                 FONT=buttonfont,$
                                                 TITLE='Wavelength Units:',$
                                                 VALUE=['um','nm','A','cm-1'],$
                                              UVALUE='Input Wavelength Units')

              state.w.iflux_dl = widget_droplist(col1_base,$
                                                 FONT=buttonfont,$
                                                 TITLE='Flux Units:',$
                                                 VALUE=['W m-2 um-1',$
                                                        'ergs s-1 cm-2 A-1',$
