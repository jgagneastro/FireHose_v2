...
;    
; PURPOSE:
;     To perform synthetic MKO-NIR photometry given a spectrum.
;
; CATEGORY:
;     Spectroscopy
;
; CALLING SEQUENCE:
;     mc_mkosyntphot,wave,flux,wunit,funit,band,mag,iflux,lave,leff,$
...
;
;-
pro  mc_mkosyntphot,wave,flux,iwunit,ifunit,band,mag,iflux,lave,leff, $
  ...
  if n_params() lt 6 then begin
     ...
     print, 'Syntax - mc_mkosyntphot,wave,flux,iwunit,ifunit,band,mag,$'
  ...
  cancel = cpar('mc_mkosyntphot',wave,'Wave',1,[1,2,3,4,5],1)
  if cancel then return
  cancel = cpar('mc_mkosyntphot',flux,'Flux',2,[1,2,3,4,5],1)
  if cancel then return
  cancel = cpar('mc_mkosyntphot',iwunit,'Iwunit',3,[1,2,3,4,5],0)
  if cancel then return
  cancel = cpar('mc_mkosyntphot',ifunit,'Fwunit',4,[1,2,3,4,5],0)
  if cancel then return
  cancel = cpar('mc_mkosyntphot',band,'Band',5,7,[0,1])
...
;  Load filter info
     ...
     case band[i] of 
        ...
        'Lp': begin
           ...
           mzp = 0.0
           readcol,filepath('atMKO_Lp.dat',ROOT_DIR=spantoolpath, $
                            SUBDIR='data'), $
                   wtrans,ttrans,FORMAT='D,D',COMMENT='#',/SILENT
        ...
        'Mp': begin
           ...
           mzp = 0.0
           readcol,filepath('atMKO_Mp.dat',ROOT_DIR=spantoolpath, $
                            SUBDIR='data'), $
                   wtrans,ttrans,FORMAT='D,D',COMMENT='#',/SILENT
...
;  sampling
     ...
     mc_interpspec,wvin,fvin,wtrans,rfvin,CANCEL=cancel
     if cancel then return
...
;  Compute values
     ...
     ivega[i] = int_tabulated(wtrans,wtrans*rfvin*ttrans)
