...
;******************************************************************************
;
pro xmc_lineparms_chunits,state,CANCEL=cancel
...
;******************************************************************************
;
pro xmc_lineparms_definenormreg,state
...
;******************************************************************************
;
pro xmc_lineparms_displayinfo,state



if not xregistered('xinfo') then begin

    state.w.xinfo_base = widget_base(GROUP_LEADER=state.w.xmc_lineparms_base, $
                                     /COLUMN,$
                                     /FLOATING,$
                                     TITLE='Xmc_Lineparms Results')
...
; Start the Event Loop. This will be a non-blocking program.
       ...
       XManager, 'xinfo', $
                 /NO_BLOCK,$
                 state.w.xinfo_base,$
                 EVENT_HANDLER='xmc_lineparms_info_event'
...
strlog[13] = ' '
strlog[14] = strtrim((*state.d.tmpdata)[8,0]*1e4,2)
...
strlog[13] = ' '
strlog[14] = strtrim((*state.d.tmpdata)[8,1]*1e4,2)
...
;  Display results
  ...
  if state.r.displayinfo then xmc_lineparms_displayinfo,state
  
  label = 'EW (A) = '+strtrim((*state.d.tmpdata)[8,0]*1e4,2)+' +- '+$
          strtrim((*state.d.tmpdata)[8,1]*1e4,2)
...
;******************************************************************************
;
pro xmc_lineparms_loadspec,state
...
;  Unfreeze widget and plot the spectrum
  ...
  state.p.freeze = 0
  xmc_lineparms_setminmax,state
  xmc_lineparms_plotupdate,state
...
;******************************************************************************
;
pro xmc_lineparms_normspec,state,CANCEL=cancel
...
;******************************************************************************
;
pro xmc_lineparms_plotnormspec,state
...
;******************************************************************************
;
pro xmc_lineparms_plotspec,state
...
pro xmc_lineparms_plotupdate,state

wset, state.p.pixmap1_wid
xmc_lineparms_plotspec,state
...
if finite((*state.d.normcoeffs)[0]) eq 1 then begin
    ...
    xmc_lineparms_plotnormspec,state
...
;******************************************************************************
;
pro xmc_lineparms_setminmax,state
...
;******************************************************************************
;
pro xmc_lineparms_stepspec,state,SKIP=skip,DONE=done
...
mc_readspec,(*state.r.files)[state.r.specidx],spc,hdr,obsmode,start,stop,$
  norders,naps,orders,xunits,yunits,slith_pix,slith_arc,slitw_pix,slitw_arc,$
  airmass,CANCEL=cancel
*state.d.w  = reform(spc[*,0])
*state.d.f  = reform(spc[*,1])
*state.d.fe = reform(spc[*,2])
...
xmc_lineparms_normspec,state
xmc_lineparms_setminmax,state
xmc_lineparms_plotupdate,state

if finite(state.r.linereg[0]) eq 1 then xmc_lineparms_ew,state
...
;******************************************************************************
;
pro xmc_lineparms_whichlinereg,state
...
;******************************************************************************
;
pro xmc_lineparms_whichnormline,state
...
;******************************************************************************
;
pro xmc_lineparms_writefile,state
...
if state.r.itype then begin
    ...
    for i = 0,n_elements(*state.r.files)-1 do begin
        ...
        printf, lun, string((*state.r.names)[i],FORMAT=format),$
                ...
                (*state.d.linedata)[8,i*2]*1e4, $
                (*state.d.linedata)[8,i*2+1]*1e4,$
    ...
    printf, lun, (*state.d.tmpdata)[0,0],(*state.d.tmpdata)[0,1],$
            ...
            (*state.d.tmpdata)[8,0]*1e4,(*state.d.tmpdata)[8,1]*1e4,$
...
;******************************************************************************
;
pro xmc_lineparms_zoom,state,IN=in,OUT=out
...
case uvalue of 
    ...
    'Done Spectrum': xmc_lineparms_stepspec,state,/DONE

    'File Button': begin

        fullpath = dialog_pickfile(DIALOG_PARENT=state.w.xmc_lineparms_base,$
                                   /MUST_EXIST)
...
;                state.r.nnormreg    = 0
            ...
            'o': xmc_lineparms_zoom,state,/OUT
    ...
    'Load File(s)': xmc_lineparms_loadspec,state

    'Next Spectrum': xmc_lineparms_stepspec,state

    'Norm Degree':state.r.normdeg = total(event.index+1)

    'Normalize Spectrum': begin

        xmc_lineparms_normspec,state,CANCEL=cancel
    ...
    'Skip Spectrum': xmc_lineparms_stepspec,state,/SKIP

    'Store EW': xmc_lineparms_storeline,state

    'Wavelength': state.r.wunits = event.index

    'Write File': xmc_lineparms_writefile,state
...
cont: 
widget_control, state.w.xmc_lineparms_base, SET_UVALUE=state, /NO_COPY
getout:
...
;******************************************************************************
;
pro xmc_lineparms_info_event,event
...
;******************************************************************************
;
pro xmc_lineparms_minmax_event,event
...
xmin  = crange(xmin1,state.p.xrange1[1],'X Min',/KLT,$
               WIDGET_ID=state.w.xmc_lineparms_base,CANCEL=cancel)
...
;******************************************************************************
;
pro xmc_lineparms_plotwin1event,event
...
case state.r.cursormode of 
    ...
    'Modify': begin
        ...
        case event.type of 
            ...
            0: begin
                ...
                xmc_lineparms_whichnormline,state
    ...
    'Norm': begin
        ...
        if count eq 0 then begin
            ...
            xmc_lineparms_definenormreg,state
            xmc_lineparms_plotupdate,state
            state.p.reg = !values.f_nan
...
;*****************************************************************************
;
pro xmc_lineparms_plotwinevent2,event
    ...
    'Modify': begin
        ...
        case event.type of 
            ...
            0: begin
                ...
                state.p.reg[*,0] = xy[0:1]
                xmc_lineparms_whichlinereg,state
...
state.w.xmc_lineparms_base = widget_base(TITLE='Xmc_Lineparms',$
                                    /COL,$
                                    /TLB_SIZE_EVENTS)

   button = widget_button(state.w.xmc_lineparms_base,$
                          FONT=buttonfont,$
                          EVENT_PRO='xmc_lineparms_event',$
                          VALUE='Done',$
                          UVALUE='Done')

   row_base = widget_base(state.w.xmc_lineparms_base,$
                          /ROW)
     ...
     col2_base = widget_base(row_base,$
         ...
         box1_base = widget_base(col2_base,$
            ...
            state.w.plotwin1 = widget_draw(box1_base,$
                                           ...
                                           /BUTTON_EVENTS,$
                                           /MOTION_EVENTS,$
                                     EVENT_PRO='xmc_lineparms_plotwin1event',$
            ...
            row = widget_base(box1_base,$
               ...
               xmin = coyote_field2(row,$
                                    ...
                                    UVALUE='X Min',$
                                    XSIZE=12,$
                                    EVENT_PRO='xmc_lineparms_minmax_event',$
                                    /CR_ONLY,$
                                    TEXTID=textid)
...
;                                  /EXCLUSIVE)
            ...
            state.w.plotwin2 = widget_draw(box2_base,$
                                           ...
                                           /BUTTON_EVENTS,$
                                           /MOTION_EVENTS,$
                                           EVENT_PRO='xmc_lineparms_plotwinevent2',$
...
centertlb,state.w.xmc_lineparms_base
widget_control, state.w.xmc_lineparms_base, /REALIZE
...
widget_geom = widget_info(state.w.xmc_lineparms_base, /GEOMETRY)
state.p.buffer[0]=widget_geom.xsize-state.p.plot1size[0]
state.p.buffer[1]=widget_geom.ysize-state.p.plot1size[1]-$
  state.p.plot2size[1]

XManager, 'xmc_lineparms', $
  /NO_BLOCK,$
  state.w.xmc_lineparms_base,$
  EVENT_HANDLER='xmc_lineparms_resize_event'

; Put state variable into the user value of the top level base.

result = dialog_message('Units must be Wm-2um-1 and um.', $
                        DIALOG_PARENT=state.w.xmc_lineparms_base)
