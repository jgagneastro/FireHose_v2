...
;
; CALLING SEQUENCE:
;     scale = mc_mkophotscale(wobj,fobj,iwunit,ifunit,mag,band,[coverage],$
;                             ERROR=error,NAN=nan,SILENT=silent,CANCEL=cancel)
;    
; INPUTS:
...
;
; KEYWORD PARAMETERS:
;     ERROR    - If given, then a Monte Carlo simulation is run using
;                the error spectrum.
...
;     2011-04-26 - Added the ERROR keyword and MC code.
;
;-
function mc_mkophotscale,wobj,fobj,iwunit,ifunit,mag,band,coverage, $
                         ERROR=error,NAN=nan,SILENT=silent,CANCEL=cancel
  ...
  if n_params() lt 6 then begin
     ...
     print, 'Syntax - scale = mc_mkophotscale(wobj,fobj,iwunit,ifunit,$'
     print, '                                 mag,band,coverage,ERROR=error,$'
     print, '                                 NAN=nan,SILENT=silent,$'
     print, '                                 CANCEL=cancel)'
     cancel = 1
     return,-1
  ...
  cancel = cpar('mc_mkophotscale',wobj,'Wobj',1,[1,2,3,4,5],1)
  if cancel then return,-1
  cancel = cpar('mc_mkophotscale',fobj,'Fobj',2,[1,2,3,4,5],1)
  if cancel then return,-1
  cancel = cpar('mc_mkophotscale',iwunit,'Iwunit',3,[1,2,3,4,5],0)
  if cancel then return,-1
  cancel = cpar('mc_mkophotscale',ifunit,'Ifunit',4,[1,2,3,4,5],0)
  if cancel then return,-1
  cancel = cpar('mc_mkophotscale',mag,'Mag',5,[1,2,3,4,5],[0,1])
  if cancel then return,-1
  cancel = cpar('mc_mkophotscale',band,'Band',6,7,[0,1])

  domc = keyword_set(ERROR)

;  Convert units to microns and W m-2 um-1

  wwobj = mc_chwunits(wobj,iwunit,0)
  ffobj = mc_chfunits(wwobj,fobj,0,ifunit,0,IERROR=error,OERROR=oerror)

;  Just for ease of coding, carry the error array around if not passed.  

  eeobj = (domc eq 1) ? oerror:make_array(1.0,n_elements(wwobj))
...
;  Remove NaNs
     ...
     if keyword_set(NAN) then begin
        ...
        ztrans = where(finite(rtrans) eq 0 or $
                       finite(tfobj) eq 0 or $
                       finite(teobj) eq 0,count)

        if count ne 0 then begin
           
           ztrans = where(finite(rtrans) eq 1 and $
                          finite(tfobj) eq 1 and $
                          finite(teobj) eq 1,count)
           tfobj  = tfobj[ztrans]
...
;  Compute intergated fluxes and determine the scale factor
     ...
     if ~domc then begin
        ...
        ndat = n_elements(tfobj)
        scales = fltarr(10000L)
        for j = 0,9999L do begin
           
           tmp = randomn(seed,ndat,/NORMAL)*teobj+tfobj          
           itfobj  = mc_int_tabulated(twobj,twobj*tmp*rtrans)

           scales[j] = (irfvin/itfobj) * 10^(-0.4*(mag[i]+mzp))

        endfor

        scale[i] = median(scales,/EVEN)
